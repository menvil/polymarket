 ...
 * ```
 */
export function toDecimal(field, input, reasonEnum, ErrorConstructor, options) {
    try {
        ...
        if (typeof input === 'number' || typeof input === 'string') {
            ...
            if (typeof obj.toString === 'function') {
                try {
                    ...
                    return Err(new ErrorConstructor(toStringError instanceof Error ? toStringError.message : 'toString() threw exception', {
                        context: {
        ...
        if (normalized === undefined) {
            return Err(new ErrorConstructor('Failed to normalize value: no valid toString()', {
                context: {
                    source: ErrorSource.PARSING,
        ...
        if (decimal.isNaN()) {
            return Err(new ErrorConstructor('Value must be finite (not NaN)', {
                context: {
                    source: ErrorSource.PARSING,
        ...
        if (!decimal.isFinite()) {
            return Err(new ErrorConstructor('Value must be finite (not Infinity or -Infinity)', {
                context: {
                    source: ErrorSource.PARSING,
                    raw: { field, value: String(normalized) },
        ...
        return Err(new ErrorConstructor(error instanceof Error ? error.message : 'Failed to parse value', {
            context: {
                source: ErrorSource.PARSING,
                raw: { field, value: safeToString(input) },
                cause: toCause(error),
 ...
 */
export function expectedMathError(e, ErrorConstructor) {
    const cause = toCause(e);
 ...
 * Трассировка (service, op, opChain) добавляется через rewrap в wrapOp.
 */
export function unexpectedError(e, ErrorConstructor) {
    const cause = toCause(e);
 ...
 * Трассировка (service, op, opChain) добавляется через rewrap в wrapOp.
 */
export function developerMisuseError(e, ErrorConstructor) {
    const cause = toCause(e);
 ...
 * Трассировка (service, op, opChain) добавляется через rewrap в wrapOp.
 */
export function coreInvariantError(e, ErrorConstructor) {
    const cause = toCause(e);
 ...
 */
export function isExpectedMathError(e) {
 ...
 * ```
 */
export function isCoreInvariantViolation(e) {
    ...
    return (e.name === 'PriceInvariantViolation' ||
 ...
 * ```
 */
export function rewrap(serviceName, op, ctx, err, ErrorConstructor, systemCtx) {
    ...
    const { cause: _c, reason: _r, raw: _raw, source: _s, service: _svc, op: _op, opChain: _chain, firstTradingErrorTimestamp: _ftets, firstTradingErrorStack: _ftes, originalName: _on, originalCode: _oc, originalErrorName: _oen, originalErrorCode: _oec, originalErrorContext: _oectx, ...safeCtx } = ctx;
    ...
    const filtered = innerChain?.filter((x) => typeof x === 'string') ?? [];
 ...
 * ```
 */
export function wrapOp(serviceName, op, ctx, fn, ErrorConstructor) {
    try {
        const result = fn();
 ...
 * ```
 */
export function currencyMismatchError(expected, actual, reasonEnum, ErrorConstructor) {
