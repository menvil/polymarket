 ...
 * ```
 */
import { Ok, Err, map as mapFn, mapErr as mapErrFn, match as matchFn, unwrapOr as unwrapOrFn, unwrapOrElse as unwrapOrElseFn, isOk, isErr, } from './result.js';
import { unwrap as unwrapFn, expectOk as expectOkFn, unwrapErr as unwrapErrFn, expectErr as expectErrFn, } from './unsafe.js';
/**
...
export class ResultChain {
    ...
    flatMap(fn) {
        if (this.data.ok) {
            return new ResultChain(fn(this.data.value));
        }
        return new ResultChain(this.data);
    }
    /**
    ...
    flatMapChain(fn) {
        if (this.data.ok) {
            return new ResultChain(fn(this.data.value).toResult());
        }
        return new ResultChain(this.data);
    }
    /**
...
//# sourceMappingURL=ResultChain.js.map