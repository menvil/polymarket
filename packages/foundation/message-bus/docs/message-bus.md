...
### Один активный drain: `_dispatching` + `_activeDrain`

Два поля с разными ролями:

- `_dispatching: boolean` — выставляется **синхронно первой строкой**
  `_runDrain()`. Проверяется в `publish()`/`publishAll()`: если drain активен,
  публикация только enqueue-ится (`Ok` = подтверждение постановки в очередь).
  Синхронность критична: reentrant-публикация из синхронного префикса первого же
  обработчика уже должна видеть активный drain, иначе запустится второй drain.
- `_activeDrain: Promise<Result<...>>` — присваивается в `_startDrain()` сразу
  после синхронного запуска `_runDrain()` (до возврата управления в event loop,
  поэтому внешние вызовы окно наблюдать не могут). Нужен `drain()`/`close()`:
  они не запускают второй drain, а дожидаются существующего и получают **тот же
  Result**, что и drain owner. Очищается в `.finally`.

Владелец drain — вызов, запустивший `_startDrain()` (publish/publishAll на idle,
drain(), close()); только он и присоединившиеся через `_activeDrain` получают
