...
- **Один активный drain**: у bus единственный drain owner. Конкурентный
  ...
  доставит существующий drain. Это верно и на границе завершения drain:
  ownership не отпускается, пока owner атомарно не убедился, что очередь пуста, —
  публикация, успевшая в microtask-окно завершения, обрабатывается тем же
  owner-ом (lost wake-up исключён и закреплён sweep-тестом).
...
(self-deadlock). Bus это misuse не детектирует. Инвариант при этом держится:
даже в самом раннем синхронном участке обработчика `drain()`/`close()`
присоединяются к активному drain — второй (nested) drain не запускается
никогда.
