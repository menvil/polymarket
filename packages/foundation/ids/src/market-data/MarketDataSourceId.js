...
export const KnownMarketDataSources = {
    ...
    POLYMARKET_REPLAY: 'POLYMARKET_REPLAY',
    // Kalshi
    KALSHI_WS: 'KALSHI_WS',
    KALSHI_REST: 'KALSHI_REST',
    KALSHI_REPLAY: 'KALSHI_REPLAY',
    // On-chain
 ...
 * @internal
 */
const KNOWN_SOURCE_SET = new Set(Object.values(KnownMarketDataSources));
/**
 * Lookup-таблица для metadata по строковому ключу (включает unknown sources → undefined)
 * @internal
 */
const SOURCE_LOOKUP = SOURCE_META;
/**
 ...
 * ```
 */
export function isKnownMarketDataSource(id) {
    if (typeof id !== 'string')
        return false;
 ...
 * ```
 */
export function asMarketDataSourceId(raw) {
 ...
 * ```
 */
export function sourceToVenue(sourceId) {
 ...
 * ```
 */
export function isLiveSource(sourceId) {
    const metadata = SOURCE_LOOKUP[sourceId];
 ...
 * ```
 */
export function isReplaySource(sourceId) {
...
//# sourceMappingURL=MarketDataSourceId.js.map