 ...
 * @internal
 */
function deepFreezeAssetId(asset) {
...
export const AssetId = {
     ...
     * ```
     */
    fromOutcomeToken(conditionRef, outcomeKey) {
        ...
        const conditionRefRaw = conditionRef;
        if (conditionRefRaw === null || typeof conditionRefRaw !== 'object') {
            return Err(new AssetIdValidationError(() => `Invalid conditionRef: must be an OnChainConditionRef object, got ${conditionRefRaw === null ? 'null' : typeof conditionRefRaw}.`, { context: { field: 'conditionRef', value: String(conditionRefRaw) } }));
        }
        // Валидация outcomeKey
        const validatedOutcomeKey = parseOutcomeKey(outcomeKey);
        if (!validatedOutcomeKey) {
            return Err(new AssetIdValidationError((ctx) => `Invalid outcomeKey: "${ctx.value}". Must be valid OutcomeKey (e.g., "UP", "DOWN").`, { context: { field: 'outcomeKey', value: String(outcomeKey) } }));
        }
        // Валидация protocolId
        const validatedProtocolId = asOnChainProtocolId(conditionRef.protocolId);
        if (!validatedProtocolId) {
            return Err(new AssetIdValidationError((ctx) => `Invalid protocolId: "${ctx.value}". Must be UPPERCASE_WITH_UNDERSCORES (e.g., "POLYMARKET_CTF").`, { context: { field: 'protocolId', value: String(conditionRef.protocolId) } }));
        }
        // Валидация chainId
        if (!isValidChainId(conditionRef.chainId)) {
            return Err(new AssetIdValidationError((ctx) => `Invalid chainId: ${ctx.value}. Must be positive integer (e.g., 137 for Polygon).`, { context: { field: 'chainId', value: conditionRef.chainId } }));
        }
        // Валидация conditionId
        const validatedConditionId = parseConditionId(conditionRef.conditionId);
        if (!validatedConditionId) {
            return Err(new AssetIdValidationError((ctx) => `Invalid conditionId: "${ctx.value}". Must be 32-byte hex string (0x...).`, { context: { field: 'conditionId', value: String(conditionRef.conditionId) } }));
        }
 ...
 * ```
 */
export function assetIdToString(asset) {
    if (asset.type === 'CURRENCY') {
 ...
 * ```
 */
export function parseAssetId(str) {
 ...
 * ```
 */
export function asPolymarketCtfToken(tokenId) {
 ...
 * Type guards
 */
export function isCurrencyAsset(asset) {
    return asset.type === 'CURRENCY';
}
export function isOutcomeTokenAsset(asset) {
    return asset.type === 'OUTCOME_TOKEN';
}
export function isPolymarketCtfToken(asset) {
