...
describe('roundToPrecision', () => {
  describe('normal operations', () => {
    ...
    it('должен округлять до целого числа', () => {
      const result = roundToPrecision(new Decimal('10.5'), 0);
    ...
    it('должен работать с очень маленькими числами', () => {
      const result = roundToPrecision(new Decimal('0.00567'), 3);
      expect(result.toString()).toBe('0.006');
    });

    it('должен не изменять уже округлённые значения', () => {
      const result = roundToPrecision(new Decimal('10.50'), 2);
  ...
  describe('edge cases', () => {
    ...
    it('должен работать с decimalPlaces = 0', () => {
      const result = roundToPrecision(new Decimal('10.9'), 0);
      expect(result.toString()).toBe('11');
    });

    it('должен работать с большим количеством знаков', () => {
      const result = roundToPrecision(new Decimal('10.123456789'), 8);
  ...
  describe('precision', () => {
    it('должен сохранять точность для больших чисел', () => {
      ...
      const result = roundToPrecision(value, 5);
