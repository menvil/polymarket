...
3. [3-слойная архитектура](#3-слойная-архитектура)
...
### 1. Почему Money НЕ имеет Rules Layer?

**Причина:** Все проверки Money — это инварианты Core, а не контекстные правила.

**Price/Quantity имеют Rules потому что:**
...
**Money НЕ имеет контекстных правил:**
- Валюта — инвариант (поддерживается или нет)
- Finite — инвариант (всегда требуется)
- MAX_AMOUNT — инвариант (всегда проверяется)
- Неотрицательность — НЕ инвариант (можно иметь отрицательный баланс)

Контекстные проверки (совпадение валют, ненулевой делитель) выполняются в Facade.
...
- **Простота** — минимум слоёв (3 вместо 4)
