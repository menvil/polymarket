...
`OutcomePrice` — цена outcome-токена рынка предсказаний, и её инвариант — жёсткий
диапазон `[0.0001, 0.9999]`:

```typescript
OutcomePriceService.create('0.42');      // ok — 42% вероятности
OutcomePriceService.create('79341.36');  // Err — OUT_OF_RANGE_HIGH
```
...
Пропустить их через `OutcomePrice` НЕЛЬЗЯ — конструктор обязан их отвергнуть. Это не
...
Верхней границы **нет** — это и есть ключевое отличие от `OutcomePrice`.
