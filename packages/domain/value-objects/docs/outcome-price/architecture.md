# Архитектура OutcomePrice Value Object
...
OutcomePrice модуль построен на принципах **Domain-Driven Design** с чётким разделением слоёв по ответственности.
...
User Code
    ↓ calls
┌─────────────────────────────────┐
│  Facade Layer (OutcomePriceService)    │
│  - Catches exceptions           │
│  - Returns Result<T, E>         │
└─────────────────────────────────┘
    ↓ calls
┌─────────────────────────────────┐
│  Core Layer (OutcomePrice)             │
...
// 1. Facade: OutcomePriceService.create()
try {
  const price = OutcomePrice.of(new Decimal(1.5));  // -> идёт в Core
  return Ok(price);
} catch (error) {
  // 2. Core: OutcomePrice.of() бросил OutcomePriceInvariantViolation
  if (error instanceof OutcomePriceInvariantViolation) {
    // 3. Facade: оборачивает в InvalidOutcomePriceError и Result
    return Err(new InvalidOutcomePriceError(...));
  }
}
```
...
┌─────────────────────────────────────────────────────┐
│  Layer 4: Adapters                                  │
│  - OutcomePriceSerializer (точная)                         │
│  - OutcomePriceFormatter (форматирование)                  │
│                                                     │
...
│                                                     │
│  Зависит от: Core (OutcomePrice — для диапазона min/max)  │
└─────────────────────────────────────────────────────┘
                        ↓
┌─────────────────────────────────────────────────────┐
│  Layer 1: Core                                      │
│  - OutcomePrice (value object)                             │
│  - OutcomePriceInvariantViolation (exception)              │
...
```typescript
// ✅ Делает: оркестрация
OutcomePriceService.complement(price)
// Внутри:
...
### Поток создания OutcomePrice

```text
User Input (number/string/Decimal)
    ↓
OutcomePriceService.create()
    ↓
try {
  OutcomePrice.of()  ← проверяет инварианты
    ↓
  constructor:
    - isNaN?       → throw OutcomePriceInvariantViolation
    - isFinite?    → throw OutcomePriceInvariantViolation
    - < MIN?       → throw OutcomePriceInvariantViolation
    - > MAX?       → throw OutcomePriceInvariantViolation
    ↓
  return Ok(price)
}
catch (OutcomePriceInvariantViolation) {
    ↓
  return Err(InvalidOutcomePriceError)
}
```
...
price
    ↓
OutcomePriceService.complement()
    ↓
...
price, tickSize, mode
    ↓
OutcomePriceService.roundToMarketTick()
    ↓
...
price, divisor
    ↓
OutcomePriceService.divide()
    ↓
parse divisor → Decimal  (try/catch)
    ↓
ValidateDivisorForPriceDivision.check(divisor)  ← Rule
    ↓
  isNaN?     → Err(InvalidOutcomePriceError)
  isFinite?  → Err(InvalidOutcomePriceError)
  isZero?    → Err(InvalidOutcomePriceError)
    ↓
...
### 2. Почему Rules не знают про OutcomePrice?

**Решение:** Rules работают только с Decimal.

**Альтернативы:**

- ❌ Rules принимают OutcomePrice — циклическая зависимость
...
```typescript
// ✅ Текущее решение
multiply(price, factor): Result<OutcomePrice, InvalidOutcomePriceError>
divide(price, divisor): Result<OutcomePrice, InvalidOutcomePriceError>

// ❌ Альтернатива
calculate(price, value, op): Result<OutcomePrice, InvalidOutcomePriceError>
```
...
if (!validateResult.ok) {
  return Err(
    rewrap('OutcomePriceService', 'divide', {
...
if (!validateResult.ok) {
  return Err(
    new InvalidOutcomePriceError(validateResult.error.message, {
      context: {
        op: 'divide',
...
**Решение:** OutcomePrice НЕ может быть 0 или 1.
...
// ❌ Generic
subtract(OutcomePrice.of(new Decimal(1)), price)
divide(add(p1, p2), 2)
```
...
public static validateSpread(
  ...
  if (!result.ok) {
    return Err(rewrap('OutcomePriceService', 'validateSpread', {
...
try {
  ...
  const price = OutcomePrice.of(decimal); // может бросить
} catch (e) {
...
const compResult = OutcomePriceService.create(compValue.toNumber());
```
...
```typescript
// Явное намерение
const compResult = OutcomePriceService.complement(price);
```
...
Архитектура OutcomePrice модуля обеспечивает:
