...
try {
  const invalid = Quantity.of(-1);  // reason: 'NEGATIVE'
} catch (e) {
  console.log(e.reason);  // 'NEGATIVE'
}

try {
  const invalid = Quantity.of(NaN);  // reason: 'NON_FINITE'
} catch (e) {
  console.log(e.reason);  // 'NON_FINITE'
}
```

#### `Quantity.fromDecimal(decimal)`

Создаёт Quantity из Decimal без повторного парсинга (zero-copy оптимизация).

```typescript
const decimal = new Decimal(10);
const qty = Quantity.fromDecimal(decimal);

// qty.value() === decimal (тот же объект!)
```
...
const big = Quantity.of("999999999999999999999");
...
```typescript
const qty1 = Quantity.of(10);
const qty2 = Quantity.of("10.0");
const qty3 = Quantity.of(5);
...
Quantity.of(0).isPositive();    // false (ноль не positive)
