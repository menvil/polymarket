...
export class Quote {
    ...
    _sourceId;
    _instrumentId;
    constructor(_bid, _ask, _bidSize, _askSize, _timestamp, _sourceId, _instrumentId) {
        this._bid = _bid;
        this._ask = _ask;
        this._bidSize = _bidSize;
        this._askSize = _askSize;
        ...
        if (_bid === null && _ask === null) {
            throw new QuoteInvariantViolation('At least one side (bid or ask) must be defined', 'BOTH_SIDES_NULL');
        }
        // Инвариант 2: bid <= ask (если оба определены)
        if (_bid !== null && _ask !== null && _bid.value().greaterThan(_ask.value())) {
            throw new QuoteInvariantViolation(`Bid ${_bid.value()} cannot be greater than ask ${_ask.value()}`, 'BID_GREATER_THAN_ASK');
        }
        // Инвариант 3: sizes >= 0
        ...
        if (_bid === null && !_bidSize.value().equals(0)) {
            throw new QuoteInvariantViolation(`Bid size must be 0 when bid is null, got ${_bidSize.value().toString()}`, 'INCONSISTENT_BID_SIZE');
        }
        // Если ask=null, то askSize должен быть 0 (нельзя иметь размер без цены)
        if (_ask === null && !_askSize.value().equals(0)) {
            throw new QuoteInvariantViolation(`Ask size must be 0 when ask is null, got ${_askSize.value().toString()}`, 'INCONSISTENT_ASK_SIZE');
        }
     ...
     * ```
     */
    static of(bid, ask, bidSize, askSize, timestamp, sourceId, instrumentId) {
    ...
    equals(other) {
        ...
        if (this._bid !== null && other._bid === null)
            return false;
        if (this._bid !== null && other._bid !== null) {
            if (!this._bid.equals(other._bid))
                return false;
        }
        // Сравниваем ask
        ...
        if (this._ask !== null && other._ask === null)
            return false;
        if (this._ask !== null && other._ask !== null) {
            if (!this._ask.equals(other._ask))
                return false;
        }
        // Сравниваем sizes
        if (!this._bidSize.equals(other._bidSize))
            return false;
        if (!this._askSize.equals(other._askSize))
            return false;
