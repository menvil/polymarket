...
export class QuoteSerializer {
    static SERVICE_NAME = 'QuoteSerializer';
    /**
    ...
    static fromJSON(json) {
        ...
        if (typeof json !== 'object' || json === null || Array.isArray(json)) {
            return Err(new InvalidQuoteError('Expected object, got ' + (json === null ? 'null' : Array.isArray(json) ? 'array' : typeof json), {
                context: {
        ...
        if (!('bid' in json)) {
            return Err(new InvalidQuoteError('Missing required field: bid', {
                context: {
                    source: ErrorSource.PARSING,
                    service: QuoteSerializer.SERVICE_NAME,
                    op,
        ...
        if (!('ask' in json)) {
            return Err(new InvalidQuoteError('Missing required field: ask', {
                context: {
                    source: ErrorSource.PARSING,
                    service: QuoteSerializer.SERVICE_NAME,
                    op,
        ...
        if (!('bidSize' in json)) {
            return Err(new InvalidQuoteError('Missing required field: bidSize', {
                context: {
                    source: ErrorSource.PARSING,
                    service: QuoteSerializer.SERVICE_NAME,
                    op,
        ...
        if (!('askSize' in json)) {
            return Err(new InvalidQuoteError('Missing required field: askSize', {
                context: {
                    source: ErrorSource.PARSING,
                    service: QuoteSerializer.SERVICE_NAME,
                    op,
        ...
        if (!('timestamp' in json)) {
            return Err(new InvalidQuoteError('Missing required field: timestamp', {
                context: {
                    source: ErrorSource.PARSING,
                    service: QuoteSerializer.SERVICE_NAME,
                    op,
        ...
        if (!('sourceId' in json)) {
            return Err(new InvalidQuoteError('Missing required field: sourceId', {
                context: {
                    source: ErrorSource.PARSING,
                    service: QuoteSerializer.SERVICE_NAME,
                    op,
        ...
        if (!('instrumentId' in json)) {
            return Err(new InvalidQuoteError('Missing required field: instrumentId', {
                context: {
                    source: ErrorSource.PARSING,
                    service: QuoteSerializer.SERVICE_NAME,
                    op,
        ...
        const { bid, ask, bidSize, askSize, timestamp, sourceId, instrumentId } = json;
        // Шаг 9: Проверка типа bid (number | null)
        if ((typeof bid !== 'number' || !Number.isFinite(bid)) && bid !== null) {
            return Err(new InvalidQuoteError(`Invalid type for field 'bid': expected finite number or null, got ${bid === null ? 'null' : typeof bid === 'number' ? String(bid) : typeof bid}`, {
                context: {
                    source: ErrorSource.PARSING,
                    service: QuoteSerializer.SERVICE_NAME,
                    op,
        ...
        if ((typeof ask !== 'number' || !Number.isFinite(ask)) && ask !== null) {
            return Err(new InvalidQuoteError(`Invalid type for field 'ask': expected finite number or null, got ${ask === null ? 'null' : typeof ask === 'number' ? String(ask) : typeof ask}`, {
                context: {
                    source: ErrorSource.PARSING,
                    service: QuoteSerializer.SERVICE_NAME,
                    op,
        ...
        if (typeof bidSize !== 'number' || !Number.isFinite(bidSize)) {
            return Err(new InvalidQuoteError(`Invalid type for field 'bidSize': expected finite number, got ${typeof bidSize === 'number' ? String(bidSize) : typeof bidSize}`, {
                context: {
                    source: ErrorSource.PARSING,
                    service: QuoteSerializer.SERVICE_NAME,
                    op,
        ...
        if (typeof askSize !== 'number' || !Number.isFinite(askSize)) {
            return Err(new InvalidQuoteError(`Invalid type for field 'askSize': expected finite number, got ${typeof askSize === 'number' ? String(askSize) : typeof askSize}`, {
                context: {
                    source: ErrorSource.PARSING,
                    service: QuoteSerializer.SERVICE_NAME,
                    op,
        ...
        if (typeof timestamp !== 'number' || !Number.isFinite(timestamp)) {
            return Err(new InvalidQuoteError(`Invalid type for field 'timestamp': expected finite number, got ${typeof timestamp === 'number' ? String(timestamp) : typeof timestamp}`, {
                context: {
                    source: ErrorSource.PARSING,
                    service: QuoteSerializer.SERVICE_NAME,
                    op,
        ...
        if (typeof sourceId !== 'string') {
            return Err(new InvalidQuoteError(`Invalid type for field 'sourceId': expected string, got ${typeof sourceId}`, {
                context: {
                    source: ErrorSource.PARSING,
                    service: QuoteSerializer.SERVICE_NAME,
                    op,
        ...
        if (typeof instrumentId !== 'string') {
            return Err(new InvalidQuoteError(`Invalid type for field 'instrumentId': expected string, got ${typeof instrumentId}`, {
                context: {
                    source: ErrorSource.PARSING,
                    service: QuoteSerializer.SERVICE_NAME,
                    op,
        ...
        return QuoteService.create(bid, ask, bidSize, askSize, sourceId, instrumentId, timestamp);
    }
    /**
    ...
    static fromJSONString(jsonString) {
        try {
            ...
            return Err(new InvalidQuoteError(`Invalid JSON string: ${error instanceof Error ? error.message : String(error)}`, {
                context: {
                    source: ErrorSource.PARSING,
                    service: QuoteSerializer.SERVICE_NAME,
                    op: 'fromJSONString',
...
//# sourceMappingURL=QuoteSerializer.js.map