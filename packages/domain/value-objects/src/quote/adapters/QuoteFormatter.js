...
export class QuoteFormatter {
     ...
     * ```
     */
    static toDisplay(quote, options = {}) {
     ...
     * ```
     */
    static toShort(quote, priceDecimals = 4) {
        const bidStr = quote.hasBid()
     ...
     * ```
     */
    static toDetailed(quote, options = {}) {
        const priceDecimals = options.priceDecimals ?? 4;
        const sizeDecimals = options.sizeDecimals ?? 2;
        ...
        const parts = [];
        if (quote.hasBid()) {
            parts.push(`Bid: ${quote.bid().value().toFixed(priceDecimals)} @ ${quote.bidSize().value().toFixed(sizeDecimals)}`);
        }
        if (quote.hasAsk()) {
            parts.push(`Ask: ${quote.ask().value().toFixed(priceDecimals)} @ ${quote.askSize().value().toFixed(sizeDecimals)}`);
        }
        ...
        if (spread !== null) {
            if (includeSpread) {
                ...
                parts.push(`Spread: ${spreadWidth.toFixed(priceDecimals)} (${spreadPct.toFixed(2)}%)`);
            }
            if (includeMid) {
     ...
     * ```
     */
    static formatSpread(quote, includePercentage = true) {
        const spread = quote.spread();
        if (spread === null) {
            return null;
        }
     ...
     * ```
     */
    static formatMid(quote, decimals = 4) {
        const spread = quote.spread();
        if (spread === null) {
            return null;
        }
     ...
     * ```
     */
    static formatCompact(quote, priceDecimals = 2, sizeDecimals = 0) {
     ...
     * ```
     */
    static formatWithSpread(quote, priceDecimals = 2) {
        if (!quote.isTwoSided()) {
            // One-sided котировка
            if (quote.hasBid()) {
...
//# sourceMappingURL=QuoteFormatter.js.map