...
import { InvalidQuoteError, ErrorSource, toDecimal, rewrap, wrapOp, unexpectedError, toCause } from '@polymarket/errors';
...
export class QuoteService {
    static SERVICE_NAME = 'QuoteService';
    /**
     ...
     */
    static parseDecimal(field, value, reason) {
     ...
     *
     */
    static parseOptionalDecimal(field, value, reason) {
        if (value === null) {
            return Ok(null);
        }
     ...
     * ```
     */
    static create(bidValue, askValue, bidSizeValue, askSizeValue, sourceId, instrumentId, timestamp) {
        const op = 'create';
        const ctx = {
        ...
        return wrapOp(QuoteService.SERVICE_NAME, op, ctx, () => {
            ...
            const bidResult = QuoteService.parseOptionalDecimal('bidValue', bidValue, QuoteErrorReason.INVALID_FORMAT);
            if (isErr(bidResult)) {
            ...
            const askResult = QuoteService.parseOptionalDecimal('askValue', askValue, QuoteErrorReason.INVALID_FORMAT);
            if (isErr(askResult)) {
            ...
            const bidSizeDecimalResult = QuoteService.parseDecimal('bidSizeValue', bidSizeValue, QuoteErrorReason.INVALID_FORMAT);
            if (isErr(bidSizeDecimalResult)) {
                return bidSizeDecimalResult;
            }
            // Конвертируем askSize через parseDecimal
            const askSizeDecimalResult = QuoteService.parseDecimal('askSizeValue', askSizeValue, QuoteErrorReason.INVALID_FORMAT);
            if (isErr(askSizeDecimalResult)) {
                return askSizeDecimalResult;
            }
            // Конвертируем timestamp через parseDecimal
            let timestampDecimal;
            if (timestamp !== undefined) {
                // Date → number (getTime)
                const tsValue = timestamp instanceof Date ? timestamp.getTime() : timestamp;
                const tsResult = QuoteService.parseDecimal('timestamp', tsValue, QuoteErrorReason.INVALID_FORMAT);
                if (isErr(tsResult)) {
                    return tsResult;
                }
            ...
            if (isErr(bidSizeQuantityResult))
                return bidSizeQuantityResult;
            const askSizeQuantityResult = this.createQuantity(askSizeDecimalResult.value, 'askSize', op);
            if (isErr(askSizeQuantityResult))
                return askSizeQuantityResult;
            ...
            if (isErr(timestampResult)) {
                return Err(new InvalidQuoteError(timestampResult.error.message, {
                    context: {
            ...
            try {
                const quote = Quote.of(bid, ask, bidSizeQuantityResult.value, askSizeQuantityResult.value, timestampResult.value, sourceId, instrumentId);
                return Ok(quote);
            }
            catch (error) {
                // Обработка инвариантов Core
                if (error instanceof QuoteInvariantViolation) {
                    return Err(new InvalidQuoteError(error.message, {
                        context: {
                ...
                // Неожиданная ошибка
                return Err(unexpectedError(error, InvalidQuoteError));
            }
        }, InvalidQuoteError);
    }
    /**
     ...
     * ```
     */
    static bidOnly(bidValue, bidSizeValue, sourceId, instrumentId, timestamp) {
        // Делегируем на create с null для ask
        return QuoteService.create(bidValue, null, // ask отсутствует
        bidSizeValue, 0, // zero ask size
        sourceId, instrumentId, timestamp);
    }
    /**
     ...
     * ```
     */
    static askOnly(askValue, askSizeValue, sourceId, instrumentId, timestamp) {
        // Делегируем на create с null для bid
        return QuoteService.create(null, // bid отсутствует
        askValue, 0, // zero bid size
        askSizeValue, sourceId, instrumentId, timestamp);
    }
    /**
     ...
     * ```
     */
    static shift(quote, shiftAmount) {
        const op = 'shift';
        const ctx = {
        ...
        return wrapOp(QuoteService.SERVICE_NAME, op, ctx, () => {
            ...
            const shiftDecimalResult = QuoteService.parseDecimal('shiftAmount', shiftAmount, QuoteErrorReason.INVALID_FORMAT);
            if (isErr(shiftDecimalResult)) {
                return shiftDecimalResult;
            }
            const shiftDecimal = shiftDecimalResult.value;
            let newBidDecimal = null;
            if (quote.bid() !== null) {
                newBidDecimal = quote.bid().value().plus(shiftDecimal);
            }
            let newAskDecimal = null;
            if (quote.ask() !== null) {
                newAskDecimal = quote.ask().value().plus(shiftDecimal);
            }
            // ВАЖНО: Сохраняем timestamp исходной котировки (нейтральная трансформация)
            return QuoteService.create(newBidDecimal, newAskDecimal, quote.bidSize().value(), quote.askSize().value(), quote.sourceId(), // Сохраняем sourceId
     ...
     * ```
     */
    static shiftWithRefresh(quote, shiftAmount, clock) {
     ...
     * ```
     */
    static skew(quote, bidAdjustment, askAdjustment) {
        const op = 'skew';
        const ctx = {
            quoteBid: quote.bid()?.value().toString() ?? null,
            quoteAsk: quote.ask()?.value().toString() ?? null,
        ...
        return wrapOp(QuoteService.SERVICE_NAME, op, ctx, () => {
            ...
            const bidAdjustmentResult = QuoteService.parseDecimal('bidAdjustment', bidAdjustment, QuoteErrorReason.INVALID_FORMAT);
            ...
            const askAdjustmentResult = QuoteService.parseDecimal('askAdjustment', askAdjustment, QuoteErrorReason.INVALID_FORMAT);
     ...
     * ```
     */
    static skewWithRefresh(quote, bidAdjustment, askAdjustment, clock) {
     ...
     * ```
     */
    static updateSizes(quote, newBidSize, newAskSize) {
        ...
        return wrapOp(QuoteService.SERVICE_NAME, op, ctx, () => {
            ...
            if (newBidSize instanceof Quantity) {
                ...
                if (isErr(bidSizeResult)) {
                    ...
                    return Err(rewrap(QuoteService.SERVICE_NAME, op, {
            ...
            try {
                const newQuote = Quote.of(quote.bid(), quote.ask(), bidSize, askSize, quote.timestamp(), quote.sourceId(), // Сохраняем sourceId
                quote.instrumentId() // Сохраняем instrumentId
                );
                return Ok(newQuote);
            }
     ...
     */
    static updateSizesWithRefresh(quote, newBidSize, newAskSize, clock) {
        ...
        return wrapOp(QuoteService.SERVICE_NAME, op, ctx, () => {
            ...
            if (isErr(newTimestampResult)) {
                return Err(new InvalidQuoteError(newTimestampResult.error.message, {
                    context: {
                        source: ErrorSource.SERVICE_CALL,
                        service: QuoteService.SERVICE_NAME,
                        op,
            ...
            try {
                const newQuote = Quote.of(quote.bid(), quote.ask(), bidSize, askSize, newTimestampResult.value, quote.sourceId(), // Сохраняем sourceId
                quote.instrumentId() // Сохраняем instrumentId
                );
                return Ok(newQuote);
            }
     ...
     * ```
     */
    static getMidPrice(quote) {
        return wrapOp(QuoteService.SERVICE_NAME, 'getMidPrice', { bid: quote.spread()?.bid()?.value()?.toString() ?? 'null', ask: quote.spread()?.ask()?.value()?.toString() ?? 'null' }, () => {
            // Validate two-sided quote (required for mid price)
            if (!quote.spread()) {
                throw new InvalidQuoteError(() => 'Cannot get mid price for one-sided quote', {
                    context: {
                        source: ErrorSource.SERVICE_CALL,
            ...
            if (isErr(spreadMidResult)) {
                ...
                throw new InvalidQuoteError((ctx) => `Cannot get mid price: ${ctx.spreadError}`, {
                    context: {
                        source: ErrorSource.SERVICE_CALL,
                        reason: QuoteErrorReason.NOT_TWO_SIDED,
     ...
     * ```
     */
    static getSpreadRatio(quote) {
        return wrapOp(QuoteService.SERVICE_NAME, 'getSpreadRatio', { bid: quote.spread()?.bid()?.value()?.toString() ?? 'null', ask: quote.spread()?.ask()?.value()?.toString() ?? 'null' }, () => {
            // Validate two-sided quote (required for spread ratio)
            if (!quote.spread()) {
                throw new InvalidQuoteError(() => 'Cannot get spread ratio for one-sided quote', {
                    context: {
                        source: ErrorSource.SERVICE_CALL,
                        reason: QuoteErrorReason.NOT_TWO_SIDED,
                        bid: quote.bid()?.value()?.toString() ?? 'null',
            ...
            if (isErr(spreadRatioResult)) {
                // Re-wrap SpreadError as QuoteError
                throw new InvalidQuoteError((ctx) => `Cannot get spread ratio: ${ctx.spreadError}`, {
                    context: {
                        source: ErrorSource.SERVICE_CALL,
                        reason: QuoteErrorReason.NOT_TWO_SIDED,
                        bid: quote.spread()?.bid()?.value()?.toString() ?? 'null',
     ...
     * ```
     */
    static shiftByRatio(quote, shiftRatio) {
        return wrapOp(QuoteService.SERVICE_NAME, 'shiftByRatio', {
            bid: quote.spread()?.bid()?.value()?.toString() ?? 'null',
            ask: quote.spread()?.ask()?.value()?.toString() ?? 'null',
            ...
            if (!quote.spread()) {
                throw new InvalidQuoteError(() => 'Cannot shift one-sided quote', {
                    context: {
                        source: ErrorSource.SERVICE_CALL,
                        reason: QuoteErrorReason.NOT_TWO_SIDED,
                        bid: quote.bid()?.value()?.toString() ?? 'null',
            ...
            if (isErr(newSpreadResult)) {
                throw new InvalidQuoteError((ctx) => `Cannot shift quote by ratio: ${ctx.spreadError}`, {
                    context: {
                        source: ErrorSource.SERVICE_CALL,
            ...
            try {
                const newQuote = Quote.of(newSpread.bid(), newSpread.ask(), quote.bidSize(), quote.askSize(), quote.timestamp(), quote.sourceId(), quote.instrumentId());
                return Ok(newQuote);
            }
            catch (error) {
                if (error instanceof QuoteInvariantViolation) {
     ...
     */
    static widenByRatio(quote, deltaWidthRatio) {
        return wrapOp(QuoteService.SERVICE_NAME, 'widenByRatio', {
            bid: quote.spread()?.bid()?.value()?.toString() ?? 'null',
            ask: quote.spread()?.ask()?.value()?.toString() ?? 'null',
            ...
            if (!quote.spread()) {
                throw new InvalidQuoteError(() => 'Cannot widen one-sided quote', {
                    context: {
                        source: ErrorSource.SERVICE_CALL,
                        reason: QuoteErrorReason.NOT_TWO_SIDED,
                        bid: quote.bid()?.value()?.toString() ?? 'null',
            ...
            if (isErr(newSpreadResult)) {
                throw new InvalidQuoteError((ctx) => `Cannot widen quote by ratio: ${ctx.spreadError}`, {
                    context: {
                        source: ErrorSource.SERVICE_CALL,
                        reason: QuoteErrorReason.RATIO_OUT_OF_BOUNDS,
                        bid: quote.spread()?.bid()?.value()?.toString() ?? 'null',
     ...
     */
    static tightenByRatio(quote, deltaWidthRatio) {
        return wrapOp(QuoteService.SERVICE_NAME, 'tightenByRatio', {
            bid: quote.spread()?.bid()?.value()?.toString() ?? 'null',
            ask: quote.spread()?.ask()?.value()?.toString() ?? 'null',
            deltaWidthRatio: deltaWidthRatio.toDecimal().toString()
        }, () => {
            ...
            if (!quote.spread()) {
                throw new InvalidQuoteError(() => 'Cannot tighten one-sided quote', {
                    context: {
                        source: ErrorSource.SERVICE_CALL,
                        reason: QuoteErrorReason.NOT_TWO_SIDED,
                        bid: quote.bid()?.value()?.toString() ?? 'null',
            ...
            if (isErr(newSpreadResult)) {
                throw new InvalidQuoteError((ctx) => `Cannot tighten quote by ratio: ${ctx.spreadError}`, {
                    context: {
                        source: ErrorSource.SERVICE_CALL,
                        reason: QuoteErrorReason.RATIO_OUT_OF_BOUNDS,
                        bid: quote.spread()?.bid()?.value()?.toString() ?? 'null',
     ...
     */
    static skewByRatio(quote, bidRatio, askRatio) {
        return wrapOp(QuoteService.SERVICE_NAME, 'skewByRatio', {
            bid: quote.spread()?.bid()?.value()?.toString() ?? 'null',
            ask: quote.spread()?.ask()?.value()?.toString() ?? 'null',
            ...
            if (!quote.spread()) {
                throw new InvalidQuoteError(() => 'Cannot skew one-sided quote', {
                    context: {
                        source: ErrorSource.SERVICE_CALL,
                        reason: QuoteErrorReason.NOT_TWO_SIDED,
                        bid: quote.bid()?.value()?.toString() ?? 'null',
            ...
            if (isErr(newSpreadResult)) {
                throw new InvalidQuoteError((ctx) => `Cannot skew quote by ratio: ${ctx.spreadError}`, {
                    context: {
                        source: ErrorSource.SERVICE_CALL,
                        reason: QuoteErrorReason.RATIO_OUT_OF_BOUNDS,
                        bid: quote.spread()?.bid()?.value()?.toString() ?? 'null',
     ...
     * ```
     */
    static scaleSizesByRatio(quote, sizeFactor) {
        return wrapOp(QuoteService.SERVICE_NAME, 'scaleSizesByRatio', {
            ...
            if (sizeFactor.toDecimal().lessThanOrEqualTo(0)) {
                throw new InvalidQuoteError(() => 'Size factor must be positive', {
                    context: {
                        source: ErrorSource.SERVICE_CALL,
            ...
            // 2. Scale bid size
            const newBidSizeResult = QuantityService.multiply(quote.bidSize(), sizeFactor.toDecimal());
            if (isErr(newBidSizeResult)) {
                throw new InvalidQuoteError((ctx) => `Cannot scale bid size: ${ctx.quantityError}`, {
                    context: {
                        source: ErrorSource.SERVICE_CALL,
                        reason: QuoteErrorReason.INVALID_FORMAT,
                        bidSize: quote.bidSize().toNumber(),
            ...
            // 3. Scale ask size
            const newAskSizeResult = QuantityService.multiply(quote.askSize(), sizeFactor.toDecimal());
            if (isErr(newAskSizeResult)) {
                throw new InvalidQuoteError((ctx) => `Cannot scale ask size: ${ctx.quantityError}`, {
                    context: {
                        source: ErrorSource.SERVICE_CALL,
                        reason: QuoteErrorReason.INVALID_FORMAT,
                        askSize: quote.askSize().toNumber(),
            ...
            if (!quote.spread()) {
                throw new InvalidQuoteError(() => 'Cannot scale sizes for one-sided quote', {
                    context: {
                        source: ErrorSource.SERVICE_CALL,
                        reason: QuoteErrorReason.NOT_TWO_SIDED,
                        bid: quote.bid()?.value()?.toString() ?? 'null',
            ...
            try {
                const newQuote = Quote.of(quote.spread().bid(), quote.spread().ask(), newBidSizeResult.value, newAskSizeResult.value, quote.timestamp(), quote.sourceId(), quote.instrumentId());
                return Ok(newQuote);
            }
            catch (error) {
                if (error instanceof QuoteInvariantViolation) {
     ...
     */
    static createPrice(value, field, op) {
        if (value === null) {
            return Ok(null);
        }
     ...
     */
    static createQuantity(value, field, op) {
