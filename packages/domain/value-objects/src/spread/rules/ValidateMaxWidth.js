...
export class ValidateMaxWidth {
    ...
    static check(width, maxWidth) {
        ...
        if (width.isNegative()) {
            return Err(new InvalidSpreadError((ctx) => `width must be non-negative, got ${ctx.width}`, {
                context: {
                    source: ErrorSource.RULE_VALIDATION,
                    width: width.toString(),
                    maxWidth: maxWidth.toString(),
        ...
        if (!maxWidth.isFinite()) {
            return Err(new InvalidSpreadError((ctx) => `maxWidth must be finite, got ${ctx.maxWidth}`, {
                context: {
                    source: ErrorSource.RULE_VALIDATION,
                    maxWidth: maxWidth.toString(),
                    width: width.toString(),
        ...
        if (maxWidth.lessThanOrEqualTo(0)) {
            return Err(new InvalidSpreadError((ctx) => `maxWidth must be positive, got ${ctx.maxWidth}`, {
                context: {
                    source: ErrorSource.RULE_VALIDATION,
                    maxWidth: maxWidth.toString(),
                    width: width.toString(),
        ...
        if (width.greaterThan(maxWidth)) {
            return Err(new InvalidSpreadError((ctx) => `Spread width ${ctx.width} exceeds maximum ${ctx.maxWidth}`, {
                context: {
                    source: ErrorSource.RULE_VALIDATION,
                    width: width.toString(),
                    maxWidth: maxWidth.toString(),
...
//# sourceMappingURL=ValidateMaxWidth.js.map