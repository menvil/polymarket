...
export class ValidateMinWidth {
     ...
     */
    static check(width, minWidth) {
        // Валидация minWidth (защита от невалидной конфигурации)
        if (!minWidth.isFinite()) {
            return Err(new InvalidSpreadError((ctx) => `minWidth must be finite, got ${ctx.minWidth}`, {
                context: {
        ...
        if (minWidth.lessThanOrEqualTo(0)) {
            return Err(new InvalidSpreadError((ctx) => `minWidth must be positive, got ${ctx.minWidth}`, {
                context: {
                    source: ErrorSource.RULE_VALIDATION,
                    minWidth: minWidth.toString(),
                    width: width.toString(),
        ...
        if (!width.isFinite()) {
            return Err(new InvalidSpreadError((ctx) => `width must be finite, got ${ctx.width}`, {
                context: {
                    source: ErrorSource.RULE_VALIDATION,
                    minWidth: minWidth.toString(),
                    width: width.toString(),
        ...
        if (width.lessThanOrEqualTo(0)) {
            return Err(new InvalidSpreadError((ctx) => `width must be positive, got ${ctx.width}`, {
                context: {
                    source: ErrorSource.RULE_VALIDATION,
                    minWidth: minWidth.toString(),
                    width: width.toString(),
        ...
        if (width.lessThan(minWidth)) {
            return Err(new InvalidSpreadError((ctx) => `Spread width ${ctx.width} is less than minimum ${ctx.minWidth}`, {
                context: {
                    source: ErrorSource.RULE_VALIDATION,
                    width: width.toString(),
                    minWidth: minWidth.toString(),
...
//# sourceMappingURL=ValidateMinWidth.js.map