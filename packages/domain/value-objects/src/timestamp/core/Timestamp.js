...
export class Timestamp {
    ...
    constructor(_ms) {
        ...
        // Инвариант 1: Not NaN
        if (_ms.isNaN()) {
            throw new TimestampInvariantViolation('Timestamp cannot be NaN', TimestampErrorReason.NOT_FINITE);
        }
        ...
        if (_ms.lessThan(0)) {
            throw new TimestampInvariantViolation(`Timestamp must be non-negative, got: ${_ms.toString()}`, TimestampErrorReason.NEGATIVE);
        }
        ...
        if (_ms.greaterThan(MAX_TIMESTAMP)) {
            throw new TimestampInvariantViolation(`Timestamp too large (must be <= ${MAX_TIMESTAMP.toString()}), got: ${_ms.toString()}`, TimestampErrorReason.OUT_OF_RANGE);
        }
        // Инвариант 5: Must be integer
        if (!_ms.isInteger()) {
            throw new TimestampInvariantViolation(`Timestamp must be integer milliseconds, got: ${_ms.toString()}`, TimestampErrorReason.NOT_INTEGER);
        }
    }
    /**
    ...
    addMs(delta) {
        ...
        if (!delta.isInteger()) {
            throw new TimestampInvariantViolation(`Delta must be integer milliseconds, got: ${delta.toString()}`, TimestampErrorReason.NOT_INTEGER);
        }
