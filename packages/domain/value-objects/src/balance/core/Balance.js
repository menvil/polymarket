...
export class Balance {
     ...
     */
    constructor(_available, _reserved, _accountId, _venueId) {
        ...
        if (totalAmount.greaterThan(Money.MAX_AMOUNT)) {
            throw new BalanceInvariantViolation(`Total balance (available + reserved) exceeds maximum: ${Money.MAX_AMOUNT}`, {
     ...
     * ```
     */
    static of(available, reserved, accountId, venueId) {
     ...
     * ```
     */
    static withZeroReserved(available, accountId, venueId) {
     ...
     * ```
     */
    static ZERO = Object.fromEntries(Object.entries(Money.ZERO).map(([currency, money]) => [
        currency,
        new Balance(money, money, { kind: 'WALLET', address: '0x0000000000000000000000000000000000000000' }, 'SYSTEM')
    ]));
    /**
