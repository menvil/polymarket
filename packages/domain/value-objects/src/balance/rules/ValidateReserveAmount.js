...
export class ValidateReserveAmount {
    static check(reserveAmount, available) {
        ...
        if (amount.lessThanOrEqualTo(0)) {
            return Err(new InvalidBalanceError((ctx) => `Reserve amount must be positive, got ${ctx.reserveAmount}`, {
                context: {
                    source: ErrorSource.RULE_VALIDATION,
                    reason: BalanceErrorReason.INVALID_FORMAT,
                    reserveAmount: amount.toString(),
        ...
        if (amount.greaterThan(availableAmount)) {
            return Err(new InvalidBalanceError((ctx) => `Cannot reserve ${ctx.requested}: only ${ctx.available} available`, {
                context: {
                    source: ErrorSource.RULE_VALIDATION,
