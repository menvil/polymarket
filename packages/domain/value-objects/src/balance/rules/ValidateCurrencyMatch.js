...
export class ValidateCurrencyMatch {
    static check(amount, balanceCurrency) {
        const amountCurrency = amount.currency();
        // Проверка: валюты должны совпадать
        if (amountCurrency !== balanceCurrency) {
            return Err(new InvalidBalanceError((ctx) => `Currency mismatch: expected ${ctx.expected}, got ${ctx.actual}`, {
                context: {
...
//# sourceMappingURL=ValidateCurrencyMatch.js.map