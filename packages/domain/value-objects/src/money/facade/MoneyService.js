...
import { InvalidMoneyError, ErrorSource, toDecimal, rewrap, wrapOp, unexpectedError } from '@polymarket/errors';
...
export class MoneyService {
    static SERVICE_NAME = 'MoneyService';
    /**
     ...
     * ```
     */
    static create(value, currency = 'USDC') {
     ...
     */
    static createFromDecimal(decimal, currency, op = 'createFromDecimal', ctx = {}) {
        try {
     ...
     */
    static mapInvariantToOverflow(op, ctx, e) {
        const { reason } = e;
        // Все типы инвариантных нарушений мапим в InvalidMoneyError с reason
        return Err(new InvalidMoneyError(`Money ${op} result is invalid: ${reason}`, {
            context: {
    ...
    static multiply(m, factor) {
        ...
        if (isErr(factorResult)) {
            return Err(rewrap(MoneyService.SERVICE_NAME, 'multiply', {
    ...
    static divide(m, divisor) {
        ...
        if (isErr(divisorResult)) {
            return Err(rewrap(MoneyService.SERVICE_NAME, 'divide', {
                amount: m.value().toString(),
    ...
    static decreaseBy(m, delta) {
        ...
        if (!result.ok) {
            return Err(rewrap(MoneyService.SERVICE_NAME, 'decreaseBy', { amount: m.value().toString(), delta: delta.toDecimal().toString(), currency: m.currency() }, result.error, InvalidMoneyError));
        }
        return result;
    }
    /**
...
//# sourceMappingURL=MoneyService.js.map