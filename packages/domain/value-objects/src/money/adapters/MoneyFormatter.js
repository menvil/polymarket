...
export class MoneyFormatter {
     ...
     * ```
     */
    static toFixed(money, decimals = 2) {
        if (decimals < 0 || !Number.isInteger(decimals)) {
            return Err(new InvalidMoneyError('decimals argument must be a non-negative integer', {
                context: {
     ...
     * ```
     */
    static toCurrency(money, showCurrency = true, decimals = 2) {
        if (decimals < 0 || !Number.isInteger(decimals)) {
            return Err(new InvalidMoneyError('decimals argument must be a non-negative integer', {
                context: {
                    source: ErrorSource.RULE_VALIDATION,
     ...
     * ```
     */
    static toCompact(money, decimals = 1) {
        if (decimals < 0 || !Number.isInteger(decimals)) {
            return Err(new InvalidMoneyError('decimals argument must be a non-negative integer', {
                context: {
                    source: ErrorSource.RULE_VALIDATION,
