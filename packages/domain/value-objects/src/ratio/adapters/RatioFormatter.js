...
export class RatioFormatter {
     ...
     * ```
     */
    static toDecimal(ratio, decimals = 4) {
        // Validate decimals (inline validation following existing pattern)
        if (decimals < 0 || !Number.isInteger(decimals)) {
            return Err(new InvalidRatioError('decimals argument must be a non-negative integer', {
                context: {
     ...
     * ```
     */
    static toPercent(ratio, decimals = 2) {
        // Validate decimals (inline validation following existing pattern)
        if (decimals < 0 || !Number.isInteger(decimals)) {
            return Err(new InvalidRatioError('decimals argument must be a non-negative integer', {
                context: {
    ...
    static parse(input) {
        ...
        if (trimmed === '') {
            return Err(new InvalidRatioError(`Invalid format: empty string`, {
                context: {
        ...
        if (trimmed.endsWith('%')) {
            ...
            if (this.hasInvalidPrefix(percentStr)) {
                return Err(new InvalidRatioError(`Invalid percent format: hex/bin/oct literals not allowed: "${input}"`, {
                    context: {
                        source: ErrorSource.PARSING,
                        op: 'parse',
                        input,
            ...
            if (!result.ok) {
                ...
                return Err(new InvalidRatioError(`Invalid percent format: "${input}"`, {
                    context: {
                        source: ErrorSource.PARSING,
                        op: 'parse',
                        input,
        ...
        if (trimmed.endsWith('bps')) {
            ...
            if (this.hasInvalidPrefix(bpsStr)) {
                return Err(new InvalidRatioError(`Invalid bps format: hex/bin/oct literals not allowed: "${input}"`, {
                    context: {
                        source: ErrorSource.PARSING,
                        op: 'parse',
                        input,
            ...
            if (!result.ok) {
                // Re-wrap error с правильным source и op
                return Err(new InvalidRatioError(`Invalid bps format: "${input}"`, {
                    context: {
                        source: ErrorSource.PARSING,
                        op: 'parse',
                        input,
        ...
        if (this.hasInvalidPrefix(trimmed)) {
            return Err(new InvalidRatioError(`Invalid decimal format: hex/bin/oct literals not allowed: "${input}"`, {
                context: {
                    source: ErrorSource.PARSING,
                    op: 'parse',
                    input,
        ...
        if (!result.ok) {
            // Re-wrap error с правильным source и op
            return Err(new InvalidRatioError(`Invalid decimal format: "${input}"`, {
                context: {
                    source: ErrorSource.PARSING,
                    op: 'parse',
                    input,
...
//# sourceMappingURL=RatioFormatter.js.map