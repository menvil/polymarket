 ...
 * ```
 */
export function validateFeeAsset(asset, ctx) {
    // 1. Должен быть непустой объект с полем type
    if (!asset || typeof asset !== 'object' || !('type' in asset)) {
        return Err(new InvalidFeeError('Invalid asset: must be object with type field', {
    ...
    if (assetObj.type !== 'CURRENCY' && assetObj.type !== 'OUTCOME_TOKEN') {
        return Err(new InvalidFeeError('Invalid asset type: must be CURRENCY or OUTCOME_TOKEN', {
            context: {
                service: ctx.service,
                op: ctx.op,
    ...
    if (assetObj.type === 'CURRENCY') {
        ...
        if (typeof currency !== 'string') {
            return Err(new InvalidFeeError('Invalid CURRENCY asset: missing or invalid currency field', {
                context: { service: ctx.service, op: ctx.op, reason: FeeErrorReason.INVALID_ASSET, currency },
            }));
        }
        if (currency.length === 0) {
            return Err(new InvalidFeeError('Invalid CURRENCY asset: currency must be non-empty string', {
                context: { service: ctx.service, op: ctx.op, reason: FeeErrorReason.INVALID_ASSET, currency },
            }));
        }
        if (!isSupportedCurrency(currency)) {
            return Err(new InvalidFeeError(`Invalid CURRENCY asset: unsupported currency '${currency}'`, {
                context: { service: ctx.service, op: ctx.op, reason: FeeErrorReason.INVALID_ASSET, currency },
            }));
        }
        return Ok(undefined);
    ...
    if (!('conditionRef' in assetObj) || !('outcomeKey' in assetObj)) {
        return Err(new InvalidFeeError('Invalid OUTCOME_TOKEN asset: missing conditionRef or outcomeKey', {
            context: {
                service: ctx.service,
                op: ctx.op,
                reason: FeeErrorReason.INVALID_ASSET,
    ...
    if (typeof conditionRef !== 'object' || conditionRef === null) {
        return Err(new InvalidFeeError('Invalid OUTCOME_TOKEN asset: conditionRef must be object', {
            context: {
                service: ctx.service,
                op: ctx.op,
                reason: FeeErrorReason.INVALID_ASSET,
    ...
    const conditionRefObj = conditionRef;
    // 4b. conditionRef должен иметь поле kind
    if (!('kind' in conditionRefObj)) {
        return Err(new InvalidFeeError('Invalid OUTCOME_TOKEN asset: conditionRef must have kind field', {
    ...
    if (typeof kind !== 'string') {
        return Err(new InvalidFeeError('Invalid OUTCOME_TOKEN asset: conditionRef.kind must be string', {
            context: {
                service: ctx.service,
                op: ctx.op,
                reason: FeeErrorReason.INVALID_ASSET,
    ...
    if (kind !== 'ONCHAIN') {
        return Err(new InvalidFeeError(`Invalid OUTCOME_TOKEN asset: unsupported conditionRef kind '${kind}'`, {
            context: {
                service: ctx.service,
                op: ctx.op,
                reason: FeeErrorReason.INVALID_ASSET,
    ...
    if (!('protocolId' in conditionRefObj) ||
        !('chainId' in conditionRefObj) ||
        !('conditionId' in conditionRefObj)) {
        return Err(new InvalidFeeError('Invalid OUTCOME_TOKEN asset: ONCHAIN conditionRef missing required fields', {
            context: {
                service: ctx.service,
                op: ctx.op,
                reason: FeeErrorReason.INVALID_ASSET,
    ...
    if (typeof protocolId !== 'string') {
        return Err(new InvalidFeeError('Invalid OUTCOME_TOKEN asset: protocolId must be string', {
            context: {
                service: ctx.service,
                op: ctx.op,
                reason: FeeErrorReason.INVALID_ASSET,
    ...
    if (asOnChainProtocolId(protocolId) === undefined) {
        return Err(new InvalidFeeError(`Invalid OUTCOME_TOKEN asset: protocolId '${protocolId}' has invalid format (must match [A-Z_][A-Z0-9_]{0,31})`, {
            context: {
                service: ctx.service,
                op: ctx.op,
                reason: FeeErrorReason.INVALID_ASSET,
    ...
    if (typeof chainId !== 'number' || !isValidChainId(chainId)) {
        return Err(new InvalidFeeError(`Invalid OUTCOME_TOKEN asset: chainId must be a positive safe integer, got ${String(chainId)}`, {
            context: {
                service: ctx.service,
                op: ctx.op,
                reason: FeeErrorReason.INVALID_ASSET,
    ...
    if (typeof conditionId !== 'string') {
        return Err(new InvalidFeeError('Invalid OUTCOME_TOKEN asset: conditionId must be string', {
            context: {
                service: ctx.service,
                op: ctx.op,
                reason: FeeErrorReason.INVALID_ASSET,
    ...
    if (!isValidConditionId(conditionId)) {
        return Err(new InvalidFeeError(`Invalid OUTCOME_TOKEN asset: conditionId '${conditionId}' has invalid format (must be 0x + 64 hex chars)`, {
            context: {
                service: ctx.service,
                op: ctx.op,
                reason: FeeErrorReason.INVALID_ASSET,
    ...
    if (typeof outcomeKey !== 'string') {
        return Err(new InvalidFeeError('Invalid OUTCOME_TOKEN asset: outcomeKey must be string', {
            context: {
                service: ctx.service,
                op: ctx.op,
                reason: FeeErrorReason.INVALID_ASSET,
    ...
    if (outcomeKey.length === 0) {
        return Err(new InvalidFeeError('Invalid OUTCOME_TOKEN asset: outcomeKey must be non-empty string', {
            context: {
                service: ctx.service,
                op: ctx.op,
                reason: FeeErrorReason.INVALID_ASSET,
    ...
    if (parseOutcomeKey(outcomeKey) === undefined) {
        return Err(new InvalidFeeError(`Invalid OUTCOME_TOKEN asset: outcomeKey '${outcomeKey}' has invalid format (control chars, ':', or '\\\\' not allowed, max 32 chars)`, {
            context: {
                service: ctx.service,
                op: ctx.op,
                reason: FeeErrorReason.INVALID_ASSET,
...
//# sourceMappingURL=validateFeeAsset.js.map