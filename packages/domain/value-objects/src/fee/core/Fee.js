...
export class Fee {
    ...
    add(other) {
        ...
        if (!AssetIdHelpers.equals(this.asset, other.asset)) {
            throw new FeeOperationError(`Cannot add fees with different assets: ${JSON.stringify(this.asset)} vs ${JSON.stringify(other.asset)}`, {
                context: {
                    operation: 'add',
        ...
        const sumDecimal = addDecimal(this._quantity.amount().value(), other._quantity.amount().value());
