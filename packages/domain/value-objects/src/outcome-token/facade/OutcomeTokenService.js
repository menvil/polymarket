...
export class OutcomeTokenService {
     ...
     * @internal
     */
    static SERVICE_NAME = 'OutcomeTokenService';
    /**
     ...
     * ```
     */
    static create(conditionRef, outcomeKey) {
        return wrapOp(OutcomeTokenService.SERVICE_NAME, 'create', { conditionRef, outcomeKey }, () => {
            // Type narrowing: OutcomeToken только для on-chain conditions
            if (conditionRef.kind !== 'ONCHAIN') {
                throw new InvalidOutcomeTokenError((ctx) => `OutcomeToken requires on-chain condition, got: ${ctx.conditionRefKind}`, {
                    context: {
...
//# sourceMappingURL=OutcomeTokenService.js.map