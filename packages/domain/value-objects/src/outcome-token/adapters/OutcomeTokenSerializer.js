...
import { parseOutcomeKey, asOnChainProtocolId, parseChainId, parseConditionId, } from '@polymarket/ids';
...
export class OutcomeTokenSerializer {
    static SERVICE_NAME = 'OutcomeTokenSerializer';
    /**
    ...
    static fromJSON(json, source = ErrorSource.PARSING) {
        ...
        if (typeof json !== 'object' || json === null) {
            return Err(new InvalidOutcomeTokenError((ctx) => `Expected object, got ${ctx.type}`, {
                context: {
                    source,
        ...
        if (Array.isArray(json)) {
            return Err(new InvalidOutcomeTokenError(() => 'Expected object, got array', {
                context: {
                    source,
                    service: OutcomeTokenSerializer.SERVICE_NAME,
                    op: 'fromJSON',
        ...
        if (!('conditionRef' in obj)) {
            return Err(new InvalidOutcomeTokenError(() => "Missing required field 'conditionRef'", {
                context: {
                    source,
                    service: OutcomeTokenSerializer.SERVICE_NAME,
                    op: 'fromJSON',
        ...
        if (!('outcomeKey' in obj)) {
            return Err(new InvalidOutcomeTokenError(() => "Missing required field 'outcomeKey'", {
                context: {
                    source,
                    service: OutcomeTokenSerializer.SERVICE_NAME,
                    op: 'fromJSON',
        ...
        if (typeof conditionRef !== 'object' || conditionRef === null || Array.isArray(conditionRef)) {
            return Err(new InvalidOutcomeTokenError((ctx) => `Field 'conditionRef' must be object, got ${ctx.type}`, {
                context: {
                    source,
                    service: OutcomeTokenSerializer.SERVICE_NAME,
                    op: 'fromJSON',
        ...
        const refObj = conditionRef;
        // Проверка полей conditionRef
        if (!('kind' in refObj) ||
            !('protocolId' in refObj) ||
            !('chainId' in refObj) ||
            !('conditionId' in refObj)) {
            return Err(new InvalidOutcomeTokenError(() => 'ConditionRef missing required fields (kind, protocolId, chainId, conditionId)', {
                context: {
                    source,
                    service: OutcomeTokenSerializer.SERVICE_NAME,
                    op: 'fromJSON',
        ...
        if (refObj.kind !== 'ONCHAIN') {
            return Err(new InvalidOutcomeTokenError((ctx) => `ConditionRef.kind must be 'ONCHAIN', got ${ctx.conditionRefKind}`, {
                context: {
                    source,
                    service: OutcomeTokenSerializer.SERVICE_NAME,
                    op: 'fromJSON',
        ...
        if (typeof refObj.protocolId !== 'string') {
            return Err(new InvalidOutcomeTokenError((ctx) => `Field 'protocolId' must be string, got ${ctx.type}`, {
                context: {
                    source,
                    service: OutcomeTokenSerializer.SERVICE_NAME,
                    op: 'fromJSON',
        ...
        if (typeof refObj.chainId !== 'number') {
            return Err(new InvalidOutcomeTokenError((ctx) => `Field 'chainId' must be number, got ${ctx.type}`, {
                context: {
                    source,
                    service: OutcomeTokenSerializer.SERVICE_NAME,
                    op: 'fromJSON',
        ...
        if (typeof refObj.conditionId !== 'string') {
            return Err(new InvalidOutcomeTokenError((ctx) => `Field 'conditionId' must be string, got ${ctx.type}`, {
                context: {
                    source,
                    service: OutcomeTokenSerializer.SERVICE_NAME,
                    op: 'fromJSON',
        ...
        if (typeof outcomeKeyValue !== 'string') {
            return Err(new InvalidOutcomeTokenError((ctx) => `Field 'outcomeKey' must be string, got ${ctx.type}`, {
                context: {
                    source,
                    service: OutcomeTokenSerializer.SERVICE_NAME,
                    op: 'fromJSON',
        ...
        if (!outcomeKey) {
            return Err(new InvalidOutcomeTokenError((ctx) => `Invalid outcomeKey format: '${ctx.outcomeKey}'`, {
                context: {
                    source,
                    service: OutcomeTokenSerializer.SERVICE_NAME,
                    op: 'fromJSON',
        ...
        if (!validatedProtocolId) {
            return Err(new InvalidOutcomeTokenError((ctx) => `Invalid protocolId format: '${ctx.protocolId}'. Must be UPPERCASE_WITH_UNDERSCORES`, {
                context: {
                    source,
                    service: OutcomeTokenSerializer.SERVICE_NAME,
                    op: 'fromJSON',
        ...
        if (!validatedChainId) {
            return Err(new InvalidOutcomeTokenError((ctx) => `Invalid chainId: ${ctx.chainId}. Must be positive integer`, {
                context: {
                    source,
                    service: OutcomeTokenSerializer.SERVICE_NAME,
                    op: 'fromJSON',
        ...
        if (!validatedConditionId) {
            return Err(new InvalidOutcomeTokenError((ctx) => `Invalid conditionId format: '${ctx.conditionId}'. Must be 32-byte hex (0x...)`, {
                context: {
                    source,
                    service: OutcomeTokenSerializer.SERVICE_NAME,
                    op: 'fromJSON',
    ...
    static toJSON(token) {
        ...
        return {
            ...
            outcomeKey: token.outcomeKey(),
        };
    }
}
//# sourceMappingURL=OutcomeTokenSerializer.js.map