...
export class Price {
  ...
  /**
   * Создаёт Price из предвалидированного значения (только для внутреннего использования)
   *
   * @param value - Значение цены (должно быть предварительно валидировано)
   * @returns Price объект без валидации
   *
   * @remarks
   * ВНИМАНИЕ: Этот метод пропускает валидацию и создаёт Price напрямую.
   * Используйте ТОЛЬКО когда математически гарантировано что значение валидно.
   *
   * Текущее использование:
   * - Spread.midpoint() - среднее двух валидных цен всегда валидно
   *
   * НЕ используйте этот метод для обычного создания Price.
   * Используйте Price.fromValue() для валидированного создания.
   *
   * @internal
   */
  public static createTrusted(value: number): Price {
