...
export class ValidateReserveAmount {
    static check(reserveQty, available) {
        ...
        if (amount.lessThanOrEqualTo(0)) {
            return Err(new InvalidTokenBalanceError((ctx) => `Reserve quantity must be positive, got ${ctx.reserveQty}`, {
                context: {
                    source: ErrorSource.RULE_VALIDATION,
                    reason: TokenBalanceErrorReason.INVALID_FORMAT,
                    reserveQty: amount.toString(),
        ...
        if (amount.greaterThan(availableAmount)) {
            return Err(new InvalidTokenBalanceError((ctx) => `Cannot reserve ${ctx.requested}: only ${ctx.available} available`, {
                context: {
                    source: ErrorSource.RULE_VALIDATION,
...
//# sourceMappingURL=ValidateReserveAmount.js.map