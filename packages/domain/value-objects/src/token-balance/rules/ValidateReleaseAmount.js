...
export class ValidateReleaseAmount {
    static check(releaseQty, reserved) {
        ...
        if (amount.lessThanOrEqualTo(0)) {
            return Err(new InvalidTokenBalanceError((ctx) => `Release quantity must be positive, got ${ctx.releaseQty}`, {
                context: {
                    source: ErrorSource.RULE_VALIDATION,
                    reason: TokenBalanceErrorReason.INVALID_FORMAT,
                    releaseQty: amount.toString(),
        ...
        if (amount.greaterThan(reservedAmount)) {
            return Err(new InvalidTokenBalanceError((ctx) => `Cannot release ${ctx.requested}: only ${ctx.reserved} reserved`, {
                context: {
                    source: ErrorSource.RULE_VALIDATION,
