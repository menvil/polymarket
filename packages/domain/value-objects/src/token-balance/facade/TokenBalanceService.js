...
export class TokenBalanceService {
    static SERVICE_NAME = 'TokenBalanceService';
    /**
     ...
     * ```
     */
    static create(token, available, reserved, accountId, venueId) {
        return wrapOp(TokenBalanceService.SERVICE_NAME, 'create', {
     ...
     * ```
     */
    static createWithZeroReserved(token, available, accountId, venueId) {
        return wrapOp(TokenBalanceService.SERVICE_NAME, 'createWithZeroReserved', {
            token: token?.assetId?.() ?? 'null',
            available: available?.value?.()?.toString() ?? 'null',
            accountId: accountId ?? 'null',
            venueId: venueId ?? 'null'
        }, () => {
            const balance = TokenBalance.withZeroReserved(token, available, accountId, venueId);
            return Ok(balance);
        }, InvalidTokenBalanceError);
    }
    /**
    ...
    static reserve(balance, qty) {
        ...
        return wrapOp(TokenBalanceService.SERVICE_NAME, op, ctx, () => {
            // Проверка: Достаточно ли available для резервирования
            const reserveCheck = ValidateReserveAmount.check(qty, balance.available());
            if (isErr(reserveCheck)) {
                return Err(rewrap(TokenBalanceService.SERVICE_NAME, op, ctx, reserveCheck.error, InvalidTokenBalanceError));
            }
            // Вычисляем новые значения
            const newAvailableResult = this.subtractQuantity(balance.available(), qty);
            if (isErr(newAvailableResult)) {
                return Err(rewrap(TokenBalanceService.SERVICE_NAME, op, ctx, newAvailableResult.error, InvalidTokenBalanceError));
            }
            const newReservedResult = this.addQuantity(balance.reserved(), qty);
            if (isErr(newReservedResult)) {
                return Err(rewrap(TokenBalanceService.SERVICE_NAME, op, ctx, newReservedResult.error, InvalidTokenBalanceError));
            }
            // Создаём новый TokenBalance (сохраняем token, accountId и venueId)
            return this.create(balance.token(), newAvailableResult.value, newReservedResult.value, balance.accountId(), balance.venueId());
        }, InvalidTokenBalanceError);
    }
    /**
     ...
     * ```
     */
    static unfreezeReserved(balance, qty) {
        ...
        return wrapOp(TokenBalanceService.SERVICE_NAME, op, ctx, () => {
            // Проверка: Достаточно ли reserved для разморозки
            const releaseCheck = ValidateReleaseAmount.check(qty, balance.reserved());
            if (isErr(releaseCheck)) {
                return Err(rewrap(TokenBalanceService.SERVICE_NAME, op, ctx, releaseCheck.error, InvalidTokenBalanceError));
            }
            // Вычисляем новые значения
            const newAvailableResult = this.addQuantity(balance.available(), qty);
            if (isErr(newAvailableResult)) {
                return Err(rewrap(TokenBalanceService.SERVICE_NAME, op, ctx, newAvailableResult.error, InvalidTokenBalanceError));
            }
            const newReservedResult = this.subtractQuantity(balance.reserved(), qty);
            if (isErr(newReservedResult)) {
                return Err(rewrap(TokenBalanceService.SERVICE_NAME, op, ctx, newReservedResult.error, InvalidTokenBalanceError));
            }
            // Создаём новый TokenBalance (сохраняем token, accountId и venueId)
     ...
     * ```
     */
    static consumeReserved(balance, qty) {
        ...
        return wrapOp(TokenBalanceService.SERVICE_NAME, op, ctx, () => {
            // Проверка: Достаточно ли reserved для списания
            const releaseCheck = ValidateReleaseAmount.check(qty, balance.reserved());
            if (isErr(releaseCheck)) {
                return Err(rewrap(TokenBalanceService.SERVICE_NAME, op, ctx, releaseCheck.error, InvalidTokenBalanceError));
            }
            ...
            return this.create(balance.token(), balance.available(), // не меняется
            newReservedResult.value, balance.accountId(), balance.venueId());
        }, InvalidTokenBalanceError);
    }
    /**
    ...
    static updateAvailable(balance, newAvailable) {
        ...
        return wrapOp(TokenBalanceService.SERVICE_NAME, op, ctx, () => {
            ...
            return this.create(balance.token(), newAvailable, balance.reserved(), // не меняется
            balance.accountId(), balance.venueId());
        }, InvalidTokenBalanceError);
    }
    /**
     ...
     * Но оставляем для будущих изменений и полноты обработки ошибок.
     */
    static subtractQuantity(qty1, qty2) {
    ...
    static equals(a, b) {
        try {
            if (!a || !b)
                return false;
            if (!(a instanceof TokenBalance) || !(b instanceof TokenBalance))
                return false;
            return a.equals(b);
        }
        catch {
