...
export class TokenBalance {
    ...
    _accountId;
    _venueId;
    constructor(_token, _available, _reserved, _accountId, _venueId) {
        this._token = _token;
        ...
        if (_available.value().isNaN() || _reserved.value().isNaN()) {
            throw new TokenBalanceInvariantViolation('TokenBalance amounts cannot be NaN', TokenBalanceErrorReason.NAN);
        }
        // Инвариант 0b: Must be finite
        if (!_available.value().isFinite() || !_reserved.value().isFinite()) {
            throw new TokenBalanceInvariantViolation('TokenBalance amounts must be finite', TokenBalanceErrorReason.NON_FINITE);
        }
        // Инвариант 1: available >= 0
        if (_available.value().isNegative()) {
            throw new TokenBalanceInvariantViolation('Available amount cannot be negative', TokenBalanceErrorReason.NEGATIVE_AVAILABLE);
        }
        // Инвариант 2: reserved >= 0
        if (_reserved.value().isNegative()) {
            throw new TokenBalanceInvariantViolation('Reserved amount cannot be negative', TokenBalanceErrorReason.NEGATIVE_RESERVED);
        }
    }
    /**
     ...
     * ```
     */
    static of(token, available, reserved, accountId, venueId) {
        // Минимальная валидация на null/undefined
        if (!token) {
            throw new TokenBalanceInvariantViolation('TokenBalance.of: token is required', TokenBalanceErrorReason.INVALID_TOKEN);
        }
        // Валидация что token это действительно OutcomeToken (не просто объект)
        if (!(token instanceof OutcomeToken)) {
            throw new TokenBalanceInvariantViolation('TokenBalance.of: token must be OutcomeToken instance', TokenBalanceErrorReason.INVALID_TOKEN);
        }
        if (!available) {
            throw new TokenBalanceInvariantViolation('TokenBalance.of: available is required', TokenBalanceErrorReason.INVALID_AMOUNT);
        }
        // Валидация что available это действительно Quantity (не просто объект)
        if (!(available instanceof Quantity)) {
            throw new TokenBalanceInvariantViolation('TokenBalance.of: available must be Quantity instance', TokenBalanceErrorReason.INVALID_AMOUNT);
        }
        if (!reserved) {
            throw new TokenBalanceInvariantViolation('TokenBalance.of: reserved is required', TokenBalanceErrorReason.INVALID_AMOUNT);
        }
        // Валидация что reserved это действительно Quantity (не просто объект)
        if (!(reserved instanceof Quantity)) {
            throw new TokenBalanceInvariantViolation('TokenBalance.of: reserved must be Quantity instance', TokenBalanceErrorReason.INVALID_AMOUNT);
        }
        if (!accountId) {
            throw new TokenBalanceInvariantViolation('TokenBalance.of: accountId is required', TokenBalanceErrorReason.INVALID_FORMAT);
        }
        if (!venueId) {
            throw new TokenBalanceInvariantViolation('TokenBalance.of: venueId is required', TokenBalanceErrorReason.INVALID_FORMAT);
        }
        // Инварианты проверяются в constructor
     ...
     * ```
     */
    static withZeroReserved(token, available, accountId, venueId) {
     ...
     */
    equals(other) {
        if (!other || !(other instanceof TokenBalance))
            return false;
        return (this._token.equals(other._token) &&
