...
export class ValidateMinSize {
    static check(quantity, minSize) {
        // Валидация minSize (защита от невалидной конфигурации)
        if (!minSize.isFinite()) {
            return Err(new InvalidQuantityError((ctx) => `minSize must be finite, got ${ctx.minSize}`, {
                context: {
        ...
        if (minSize.lessThanOrEqualTo(0)) {
            return Err(new InvalidQuantityError((ctx) => `minSize must be positive, got ${ctx.minSize}`, {
                context: {
                    source: ErrorSource.RULE_VALIDATION,
                    minSize: minSize.toString(),
                    quantity: quantity.toString()
        ...
        if (quantity.lessThan(minSize)) {
            return Err(new InvalidQuantityError((ctx) => `Quantity ${ctx.quantity} is less than minimum size ${ctx.minSize}`, {
                context: {
                    source: ErrorSource.RULE_VALIDATION,
                    quantity: quantity.toString(),
                    minSize: minSize.toString()
...
//# sourceMappingURL=ValidateMinSize.js.map