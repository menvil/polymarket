...
export class ValidateFactorForQuantityMultiplication {
    static check(factor) {
        ...
        if (!factor.isFinite()) {
            return Err(new InvalidQuantityError((ctx) => `Factor for Quantity multiplication must be finite, got ${ctx.factor}`, {
                context: {
        ...
        if (factor.lessThan(new Decimal(0))) {
            return Err(new InvalidQuantityError((ctx) => `Factor for Quantity multiplication cannot be negative, got ${ctx.factor}`, {
                context: {
                    source: ErrorSource.RULE_VALIDATION,
                    factor: factor.toString()
                }
...
//# sourceMappingURL=ValidateFactorForQuantityMultiplication.js.map