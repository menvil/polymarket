import { Ok, isErr } from '@polymarket/result';
...
export class AssetQuantityService {
    static SERVICE_NAME = 'AssetQuantityService';
    /**
    ...
    static create(asset, amount) {
        return wrapOp(AssetQuantityService.SERVICE_NAME, 'create', { asset, amount }, () => {
     ...
     * ```
     */
    static createUsdc(amountValue) {
        return wrapOp(AssetQuantityService.SERVICE_NAME, 'createUsdc', { amountValue }, () => {
            ...
            if (isErr(quantityResult)) {
                ...
                throw new InvalidAssetQuantityError((ctx) => `Invalid amount for USDC: ${ctx.quantityError}`, {
                    context: {
     ...
     * ```
     */
    static createOutcomeToken(conditionRef, outcomeKey, amountValue) {
        return wrapOp(AssetQuantityService.SERVICE_NAME, 'createOutcomeToken', { conditionRef, outcomeKey, amountValue }, () => {
            // Используем QuantityService для парсинга и валидации (DRY, централизация)
            const quantityResult = QuantityService.create(amountValue);
            if (isErr(quantityResult)) {
                // Переупаковываем ошибку QuantityService в InvalidAssetQuantityError
                throw new InvalidAssetQuantityError((ctx) => `Invalid amount for outcome token: ${ctx.quantityError}`, {
                    context: {
                        source: ErrorSource.SERVICE_CALL,
                        reason: AssetQuantityErrorReason.INVALID_AMOUNT,
                        conditionRef,
            ...
            try {
                ...
                throw new InvalidAssetQuantityError(() => errorMessage, {
                    context: {
                        source: ErrorSource.SERVICE_CALL,
    ...
    static portion(assetQty, rate) {
        ...
        return wrapOp(AssetQuantityService.SERVICE_NAME, 'portion', ctx, () => {
            ...
            if (isErr(quantityResult)) {
                // Переупаковываем ошибку QuantityService в InvalidAssetQuantityError
                throw new InvalidAssetQuantityError((ctx) => `Invalid result amount for portion: ${ctx.quantityError}`, {
                    context: {
                        source: ErrorSource.SERVICE_CALL,
                        reason: AssetQuantityErrorReason.INVALID_AMOUNT,
                        amount: assetQty.amount().value().toString(),
...
//# sourceMappingURL=AssetQuantityService.js.map