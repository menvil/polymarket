...
export class ValidateTickSize {
     ...
     * ```
     */
    static check(tickSize) {
        // Проверка NaN
        if (tickSize.isNaN()) {
            return Err(new InvalidTickSizeError(() => `Tick size must not be NaN`, {
                context: {
        ...
        if (!tickSize.isFinite()) {
            return Err(new InvalidTickSizeError((ctx) => `Tick size must be finite, got ${ctx.tickSize}`, {
                context: {
                    source: ErrorSource.RULE_VALIDATION,
                    field: 'tickSize',
                    reason: 'not_finite',
        ...
        if (tickSize.lessThanOrEqualTo(0)) {
            return Err(new InvalidTickSizeError((ctx) => `Tick size must be positive, got ${ctx.tickSize}`, {
                context: {
                    source: ErrorSource.RULE_VALIDATION,
                    field: 'tickSize',
                    reason: 'not_positive',
        ...
        if (tickSize.greaterThan(maxAllowed)) {
            return Err(new InvalidTickSizeError((ctx) => `Tick size ${ctx.tickSize} exceeds price range`, {
                context: {
                    source: ErrorSource.RULE_VALIDATION,
                    field: 'tickSize',
...
//# sourceMappingURL=ValidateTickSize.js.map