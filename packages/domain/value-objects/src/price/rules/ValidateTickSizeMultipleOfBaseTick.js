...
export class ValidateTickSizeMultipleOfBaseTick {
    ...
    static check(tickSize) {
        ...
        if (!quotient.isInteger()) {
            return Err(new InvalidTickSizeError((ctx) => `Tick size ${ctx.tickSize} must be multiple of base tick ${ctx.baseTick}`, {
                context: {
                    source: ErrorSource.RULE_VALIDATION,
                    field: 'tickSize',
                    reason: 'not_multiple_of_base_tick',
...
//# sourceMappingURL=ValidateTickSizeMultipleOfBaseTick.js.map