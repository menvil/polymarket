...
import { addDecimal, subtractDecimal, multiplyDecimal, divideDecimal, roundToTick, floorToTick, ceilToTick } from '@polymarket/math';
...
export class PriceService {
    static SERVICE_NAME = 'PriceService';
    /**
    ...
    static create(value) {
        ...
        return wrapOp(PriceService.SERVICE_NAME, 'create', { raw: { field: 'value', value: String(value) } }, () => {
    ...
    static multiply(price, factor) {
        ...
        if (isErr(factorResult)) {
            return Err(rewrap(PriceService.SERVICE_NAME, 'multiply', {
    ...
    static divide(price, divisor) {
        ...
        if (isErr(divisorResult)) {
            return Err(rewrap(PriceService.SERVICE_NAME, 'divide', {
                price: price.value().toString(),
     ...
     * ```
     */
    static roundToMarketTick(price, tickSize, mode = 'nearest') {
        ...
        if (isErr(tickDecimalResult)) {
            return Err(rewrap(PriceService.SERVICE_NAME, 'roundToMarketTick', {
                price: price.value().toString(),
     ...
     * ```
     */
    static ensureAlignedToMarketTick(price, tickSize) {
        // Безопасный парсинг tickSize через toDecimal
        const tickDecimalResult = toDecimal('tickSize', tickSize, PriceErrorReason.INVALID_FORMAT, InvalidPriceError);
        if (isErr(tickDecimalResult)) {
            return Err(rewrap(PriceService.SERVICE_NAME, 'ensureAlignedToMarketTick', {
                price: price.value().toString(),
                tickSize: String(tickSize)
            }, tickDecimalResult.error, InvalidPriceError));
        }
     ...
     * ```
     */
    static applyRelativeChange(price, ratio, tickSize, options) {
        const roundingMode = options?.roundingMode ?? 'nearest';
        // Безопасный парсинг tickSize через toDecimal
        const tickDecimalResult = toDecimal('tickSize', tickSize, PriceErrorReason.INVALID_FORMAT, InvalidPriceError);
        if (isErr(tickDecimalResult)) {
            return Err(rewrap(PriceService.SERVICE_NAME, 'applyRelativeChange', {
                price: price.value().toString(),
