...
describe('MoneyFormatter', () => {
  ...
  describe('toCompact', () => {
    ...
    it('форматирует граничные значения', () => {
      expect(MoneyFormatter.toCompact(Money.of(new Decimal(1000)))).toBe('$1.0K');
      expect(MoneyFormatter.toCompact(Money.of(new Decimal(1000000)))).toBe('$1.0M');
      expect(MoneyFormatter.toCompact(Money.of(new Decimal(1000000000)))).toBe('$1.0B');
    });
