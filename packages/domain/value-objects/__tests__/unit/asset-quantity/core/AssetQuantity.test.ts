...
describe('AssetQuantity Core', () => {
  ...
  if (!tokenAssetResult.ok) throw tokenAssetResult.error;
  const tokenAsset = tokenAssetResult.value;
  ...
  describe('fromAssetId() factory', () => {
    it('создаёт AssetQuantity для Currency через fromAssetId (frozen AssetId)', () => {
      const assetQty = AssetQuantity.fromAssetId(usdcAsset, qty100);
    ...
    it('создаёт AssetQuantity для OutcomeToken через fromAssetId (frozen AssetId)', () => {
      const assetQty = AssetQuantity.fromAssetId(tokenAsset, qty200);

      expect(assetQty).toBeInstanceOf(AssetQuantity);
      expect(assetQty.amount()).toBe(qty200);
      expect(assetQty.isOutcomeToken()).toBe(true);
    });
  });

  describe('конструктор — defensive copy для не-frozen AssetId', () => {
    it('пересоздаёт Currency AssetId если он не frozen', () => {
      // Мутабельный (не-frozen) AssetId — симулирует parseAssetId()
      const mutableCurrencyAsset = { type: 'CURRENCY' as const, currency: 'USDC' as const };
      expect(Object.isFrozen(mutableCurrencyAsset)).toBe(false);

      const assetQty = new AssetQuantity(mutableCurrencyAsset, qty100);

      expect(assetQty.isCurrency()).toBe(true);
      expect(assetQty.amount()).toBe(qty100);
      // Внутренний asset заменён на канонический (frozen)
      expect(Object.isFrozen(assetQty.asset())).toBe(true);
    });

    it('пересоздаёт OutcomeToken AssetId если он не frozen', () => {
      // Мутабельный AssetId для outcome token
      const mutableTokenAsset = {
        type: 'OUTCOME_TOKEN' as const,
        conditionRef: conditionRef,
        outcomeKey: BinaryOutcome.UP,
      };
      expect(Object.isFrozen(mutableTokenAsset)).toBe(false);

      const assetQty = new AssetQuantity(mutableTokenAsset, qty100);
    ...
    it('использует frozen AssetId напрямую без пересоздания', () => {
      // AssetIdHelpers возвращает frozen AssetId
      expect(Object.isFrozen(usdcAsset)).toBe(true);

      const assetQty = new AssetQuantity(usdcAsset, qty100);

      // Frozen AssetId используется напрямую (тот же объект)
      expect(assetQty.asset()).toBe(usdcAsset);
    });
  });
