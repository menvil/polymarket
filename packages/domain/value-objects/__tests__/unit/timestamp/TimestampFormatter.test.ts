...
import { Timestamp, TimestampFormatter } from '../../../src/timestamp/index.js';
import { unwrap } from '@polymarket/result/unsafe';

describe('TimestampFormatter', () => {
  const testTimestamp = unwrap(Timestamp.fromEpochMs(1705318200000)); // 2024-01-15T10:30:00.000Z
  ...
  describe('toRelative()', () => {
    it('should format seconds ago', () => {
      const now = unwrap(Timestamp.fromEpochMs(Date.now()));
      const past = unwrap(Timestamp.fromEpochMs(Date.now() - 30000)); // 30 seconds ago
    ...
    it('should format minutes ago', () => {
      const now = unwrap(Timestamp.fromEpochMs(Date.now()));
      const past = unwrap(Timestamp.fromEpochMs(Date.now() - 120000)); // 2 minutes ago
    ...
    it('should format hours ago', () => {
      const now = unwrap(Timestamp.fromEpochMs(Date.now()));
      const past = unwrap(Timestamp.fromEpochMs(Date.now() - 7200000)); // 2 hours ago
    ...
    it('should format days ago', () => {
      const now = unwrap(Timestamp.fromEpochMs(Date.now()));
      const past = unwrap(Timestamp.fromEpochMs(Date.now() - 172800000)); // 2 days ago
    ...
    it('should format future timestamp with "in"', () => {
      const now = unwrap(Timestamp.fromEpochMs(Date.now()));
      const future = unwrap(Timestamp.fromEpochMs(Date.now() + 30000)); // in 30 seconds
    ...
    it('should use singular form for 1 second', () => {
      const now = unwrap(Timestamp.fromEpochMs(Date.now()));
      const past = unwrap(Timestamp.fromEpochMs(Date.now() - 1000)); // 1 second ago
    ...
    it('should use plural form for multiple seconds', () => {
      const now = unwrap(Timestamp.fromEpochMs(Date.now()));
      const past = unwrap(Timestamp.fromEpochMs(Date.now() - 5000)); // 5 seconds ago
