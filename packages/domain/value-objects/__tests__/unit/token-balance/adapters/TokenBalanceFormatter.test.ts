...
describe('TokenBalanceFormatter', () => {
  ...
  describe('toString()', () => {
    it('форматирует TokenBalance как полную строку', () => {
      ...
      expect(str).toContain('instrument-up');
  ...
  describe('toDisplayString()', () => {
    it('форматирует TokenBalance для UI', () => {
      ...
      // Раньше здесь ожидались on-chain подробности (протокол, chainId,
      // сокращённый conditionId): формат печатал `OutcomeToken`. Теперь VO
      // ключуется `InstrumentId`, и в строку идёт он — off-chain площадка
      // conditionRef не имеет вовсе.
      expect(display).toContain('100.5');
      expect(display).toContain('instrument-up');
    });
  ...
  describe('toShortString()', () => {
    it('форматирует TokenBalance в краткую строку', () => {
      ...
      expect(short).toBe('100.5 instrument-up'); // available=100.5, reserved=0, total=100.5
    });
  ...
  describe('toVerboseString()', () => {
    it('форматирует TokenBalance с полной информацией', () => {
      ...
      expect(verbose).toContain('instrument-up');
  ...
  describe('toFixedString()', () => {
    ...
    it('округляет amount при необходимости', () => {
      ...
      expect(fixed2).toBe('100.12 instrument-up');
    });
    ...
    it('форматирует нулевой баланс', () => {
      ...
      expect(fixed).toBe('0.00 instrument-up');
    });
  ...
  describe('toSummary()', () => {
    it('форматирует полную сводку баланса', () => {
      ...
      expect(summary).toContain('[instrument-up]');
    });
  ...
  describe('toDebugString()', () => {
    it('форматирует debug строку с полной информацией', () => {
      ...
      expect(debug).toContain('token: instrument-up');
