...
describe('OutcomePrice Integration Tests', () => {
  describe('roundToMarketTick → ValidateAligned контракт', () => {
    it('результат roundToMarketTick всегда проходит ensureAlignedToMarketTick', () => {
      ...
      const roundResult = OutcomePriceService.roundToMarketTick(price, tickSize);
      expect(roundResult.ok).toBe(true);

      if (roundResult.ok) {
        const alignResult = OutcomePriceService.ensureAlignedToMarketTick(roundResult.value, tickSize);
    ...
    it('работает для всех режимов округления', () => {
      ...
      for (const mode of ['nearest', 'floor', 'ceil'] as const) {
        const roundResult = OutcomePriceService.roundToMarketTick(price, tickSize, mode);
        expect(roundResult.ok).toBe(true);
  ...
  describe('Serialization round-trip', () => {
    it('fromJSON → toJSON сохраняет значение', () => {
      ...
      const deserResult = OutcomePriceSerializer.fromJSON(original);
      expect(deserResult.ok).toBe(true);

      if (deserResult.ok) {
        const serialized = OutcomePriceSerializer.toJSON(deserResult.value);
        expect(serialized.value).toBe('0.5');

        const deserResult2 = OutcomePriceSerializer.fromJSON(serialized);
  ...
  describe('Полный workflow: создание → операции → округление → валидация', () => {
    it('создание bid/ask с округлением', () => {
      ...
      // Шаг 2: Создаём OutcomePrice
      const bidResult = OutcomePriceService.create(rawBid);
      const askResult = OutcomePriceService.create(rawAsk);
      expect(bidResult.ok).toBe(true);
      expect(askResult.ok).toBe(true);

      if (bidResult.ok && askResult.ok) {
        // Шаг 3: Округляем
        const roundedBidResult = OutcomePriceService.roundToMarketTick(bidResult.value, tickSize, 'floor');
        const roundedAskResult = OutcomePriceService.roundToMarketTick(askResult.value, tickSize, 'ceil');
        ...
        if (roundedBidResult.ok && roundedAskResult.ok) {
          ...
          expect(OutcomePriceService.ensureAlignedToMarketTick(roundedBidResult.value, tickSize).ok).toBe(true);
          expect(OutcomePriceService.ensureAlignedToMarketTick(roundedAskResult.value, tickSize).ok).toBe(true);

          // Шаг 5: Вычисляем mid-price
          const midResult = OutcomePriceService.average(roundedBidResult.value, roundedAskResult.value);
  ...
  describe('Decimal configuration', () => {
    it('Decimal precision достаточна для OutcomePrice операций', () => {
    ...
    it('Decimal arithmetic точна для OutcomePrice диапазона', () => {
  ...
  describe('multiply/divide семантика', () => {
    it('multiply с InvalidOutcomePriceError для невалидного factor', () => {
  ...
  describe('Adapters unknown валидация', () => {
    it('fromJSON валидирует структуру перед делегированием', () => {
      ...
      for (const invalid of invalidStructures) {
        const result = OutcomePriceSerializer.fromJSON(invalid);
        expect(result.ok).toBe(false);
        if (!result.ok) {
    ...
    it('fromJSON делегирует бизнес-валидацию OutcomePriceService', () => {
      const outOfRange = [
        { value: 1.5 },
        { value: -0.5 },
        { value: 0.00001 }
