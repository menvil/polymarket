 ...
 * ```
 */
export function getTotalValue(positions, getPrice, currency) {
 ...
 * ```
 */
export function getTotalUnrealizedPnL(positions, getPrice) {
    let total = new Decimal(0);
    for (const position of positions) {
        const price = getPrice(position.instrumentId);
        if (!price)
...
//# sourceMappingURL=PortfolioValuationService.js.map