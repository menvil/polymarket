...
/**
 * Статусы, в которых заявка считается живой экспозицией.
 *
 * @remarks
 * Перечислены ЯВНО, а не выведены как дополнение {@link TERMINAL_STATUSES}.
 * Дело не в текущем составе — сегодня это в точности дополнение, и тест
 * полноты за этим следит. Дело в том, что новый нетерминальный статус,
 * добавленный сюда завтра, при выводе через отрицание молча стал бы
 * «открытым». Явный список сломает тест полноты и потребует решения.
 *
 * От {@link FILLABLE_STATUSES} отличается ровно на `PENDING`, и это
 * СОЗНАТЕЛЬНО: принять исполнение отправленная, но не подтверждённая заявка
 * ещё не может, а вот деньги или токены под неё уже зарезервированы. Для
 * риска это принятое обязательство независимо от того, ответила площадка или
 * нет.
 */
