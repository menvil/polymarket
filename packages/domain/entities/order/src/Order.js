...
import { TERMINAL_STATUSES, FILLABLE_STATUSES, } from './OrderState.js';
...
export class Order {
    ...
    /** Торгуемый актив */
    get asset() { return this._s.asset; }
    /** Сторона (BUY/SELL) */
    get side() { return this._s.side; }
    /** Лимитная цена заявки */
    get price() { return this._s.price; }
    ...
    get reason() { return this._s.reason; }
    /** ID стратегии (для изоляции multi-strategy) */
    get strategyId() { return this._s.strategyId; }
    // ─── Fill state ────────────────────────────────────────────────────────────
    /** Внутреннее состояние fills (filledSize, averagePrice, fillIds) */
    get fill() { return this._s.fill; }
    /** Исполненный объём */
    get filledSize() { return this._s.fill.filledSize; }
    /** Средневзвешенная цена исполнения (VWAP), undefined если нет fills */
    get averagePrice() { return this._s.fill.averagePrice; }
    /** Список ID всех применённых fills */
    get fillIds() { return this._s.fill.fillIds; }
    /** Количество применённых fills */
    get tradeCount() { return this._s.fill.fillIds.length; }
    // ─── Computed ──────────────────────────────────────────────────────────────
    /**
    ...
    get isTerminal() { return TERMINAL_STATUSES.has(this._s.status); }
    /** true если заявка может принимать fills (OPEN/PARTIALLY_FILLED) */
    get isFillable() { return FILLABLE_STATUSES.has(this._s.status); }
    // ─── Status predicates ─────────────────────────────────────────────────────
    isPending() { return this._s.status === 'PENDING'; }
    isOpen() { return this._s.status === 'OPEN'; }
    isFilled() { return this._s.status === 'FILLED'; }
    isPartiallyFilled() { return this._s.status === 'PARTIALLY_FILLED'; }
    canCancel() { return FILLABLE_STATUSES.has(this._s.status); }
    canModify() { return !TERMINAL_STATUSES.has(this._s.status); }
    // ─── Domain Event Outbox ───────────────────────────────────────────────────
    /**
    ...
    static create(params) {
        ...
        return Ok(new Order(Order._applyEventToState({}, event), [event]));
    }
    // ─── Factory: rehydrate ────────────────────────────────────────────────────
    /**
    ...
    static rehydrate(state) {
        ...
        if (filledVal.gt(sizeVal)) {
            return Err(new TradingError(`filledSize (${filledVal}) exceeds size (${sizeVal})`, { context: { orderId: state.id, filledSize: filledVal.toString(), size: sizeVal.toString() } }));
        }
        if (state.status === 'PENDING' && !filledVal.isZero()) {
            return Err(new TradingError(`PENDING order cannot have fills (filledSize: ${filledVal})`, { context: { orderId: state.id, filledSize: filledVal.toString() } }));
        }
        if (state.status === 'FILLED' && !filledVal.eq(sizeVal)) {
            return Err(new TradingError(`FILLED order must have filledSize equal to size (filledSize: ${filledVal}, size: ${sizeVal})`, { context: { orderId: state.id, filledSize: filledVal.toString(), size: sizeVal.toString() } }));
        }
        if (state.status === 'PARTIALLY_FILLED' && !(filledVal.gt(0) && filledVal.lt(sizeVal))) {
            return Err(new TradingError(`PARTIALLY_FILLED order must have 0 < filledSize < size (filledSize: ${filledVal}, size: ${sizeVal})`, { context: { orderId: state.id, filledSize: filledVal.toString(), size: sizeVal.toString() } }));
        }
    ...
    static fromEvents(events) {
        ...
        if (first.type !== 'ORDER_CREATED') {
            return Err(new TradingError(`First event must be ORDER_CREATED, got ${first.type}`, { context: { eventType: first.type } }));
        }
        let state = {
            id: first.orderId,
            asset: first.asset,
     ...
     */
    static _applyEventToState(state, event) {
        // Защита от чужих событий: игнорируем если orderId не совпадает (кроме ORDER_CREATED)
        if (event.type !== 'ORDER_CREATED' && event.orderId !== state.id)
            return state;
        switch (event.type) {
            case 'ORDER_CREATED':
                return {
            ...
            case 'ORDER_CANCELLED':
                if (!FILLABLE_STATUSES.has(state.status))
                    return state;
            ...
            case 'ORDER_FILLED': {
                ...
                const newStatus = event.type === 'ORDER_FILLED' ? 'FILLED' : 'PARTIALLY_FILLED';
    ...
    accept() {
        if (this._s.status !== 'PENDING') {
            return Err(new TradingError(`Cannot accept order with status ${this._s.status}. Only PENDING orders can be accepted.`, { context: { orderId: this._s.id } }));
        }
        const event = { type: 'ORDER_ACCEPTED', orderId: this._s.id };
    ...
    reject(reason) {
        ...
        if (this._s.status !== 'PENDING') {
            return Err(new TradingError(`Cannot reject order with status ${this._s.status}. Only PENDING orders can be rejected.`, { context: { orderId: this._s.id } }));
        }
        const event = { type: 'ORDER_REJECTED', orderId: this._s.id, reason };
        return Ok(new Order(Order._applyEventToState(this._s, event), [event]));
    }
    /**
     * Отменить заявку
    ...
    cancel(reason) {
        if (!FILLABLE_STATUSES.has(this._s.status)) {
            return Err(new TradingError(`Cannot cancel order with status ${this._s.status}. Only OPEN or PARTIALLY_FILLED orders can be cancelled.`, { context: { orderId: this._s.id } }));
        }
        const cancelReason = reason ?? 'User cancelled';
        const event = { type: 'ORDER_CANCELLED', orderId: this._s.id, reason: cancelReason };
        return Ok(new Order(Order._applyEventToState(this._s, event), [event]));
    }
    /**
    ...
    expire() {
        if (!FILLABLE_STATUSES.has(this._s.status)) {
            return Err(new TradingError(`Cannot expire order with status ${this._s.status}. Only OPEN or PARTIALLY_FILLED orders can expire.`, { context: { orderId: this._s.id } }));
        }
        const event = { type: 'ORDER_EXPIRED', orderId: this._s.id };
        return Ok(new Order(Order._applyEventToState(this._s, event), [event]));
    }
    /**
    ...
    applyFill(fill) {
        if (!FILLABLE_STATUSES.has(this._s.status)) {
            return Err(new TradingError(`Cannot apply fill to order with status ${this._s.status}. Only OPEN or PARTIALLY_FILLED orders can accept fills.`, { context: { orderId: this._s.id, fillId: fill.id } }));
        }
        ...
        if (fill.side !== this._s.side) {
            return Err(new TradingError(`Fill side (${fill.side}) does not match order side (${this._s.side})`, { context: { orderId: this._s.id, fillId: fill.id } }));
        }
        if (fill.orderId !== this._s.id) {
            return Err(new TradingError(`Fill orderId (${fill.orderId}) does not match this order id (${this._s.id})`, { context: { orderId: this._s.id, fillId: fill.id } }));
        }
     ...
     */
    canAcceptFill(fill) {
        return (FILLABLE_STATUSES.has(this._s.status) &&
    ...
    toSnapshot() {
        return {
            ...
            fillIds: this._s.fill.fillIds.map(id => id),
