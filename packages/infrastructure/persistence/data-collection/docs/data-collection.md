...
- `'EXPIRED'` — завершённый dataset: flush буфера → корректное закрытие
  ...
  завершённый архив реально создан; отказ сжатия логируется как error и
  ПРОБРАСЫВАЕТСЯ (false success запрещён — вызывающий finalizer не снимает
  FINALIZING-сессию и не считает архив). Retry сжатия не выполняется;
  оставшийся `.jsonl` заберёт стандартный cleanup незавершённых файлов.
