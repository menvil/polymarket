...
Сравнение идёт по `OutcomePrice.equals` (Decimal), а не по строкам.
...
Production-потребителей у прежнего значения не было (legacy
`BookUpdateHandler` подставлял туда timestamp как заглушку).

Поэтому номер **per-instrument** и монотонный. Но считает он именно
ОПУБЛИКОВАННЫЕ СОБЫТИЯ верхушки, а не применённые обновления книги — и это
исправление более поздней ревизии.

### Почему не версия книги

Первая версия подставляла сюда версию реконструкции
(`OrderbookReconstructionState.version`), которая растёт на КАЖДОМ
применённом обновлении. При этом `BOOK_UPDATED` публикуется только при смене
верхушки, поэтому глубинные правки «съедали» номера:

```text
book          верхушка изменилась  → BOOK_UPDATED seq=1
price_change  задет уровень ниже   → BOOK_UPDATED НЕТ, версия книги = 2
price_change  верхушка изменилась  → BOOK_UPDATED seq=3
```

Подписчик видел ряд `1, 3` и по контракту поля обязан прочитать это как
ПОТЕРЮ события 2. То есть поле, заведённое ради gap detection, само
производило ложные gap-ы. Для Polymarket это особенно заметно: дельты
`price_change` частые и сплошь и рядом задевают только глубину.

Теперь действуют два правила:
...
- отказ шины номер **тоже не тратит**: если публикация номера N провалилась,
  следующая попытка получит снова N, а не N+1.

Номер и отпечаток верхушки хранятся ОДНОЙ записью на инструмент и
продвигаются вместе, только после успешной публикации: две параллельные мапы
разъехались бы при первом же cleanup-е, забывшем одну из них.

### Версия книги никуда не делась

`OrderbookReconstructionState.version` остаётся версией КНИГИ и по-прежнему
растёт на каждое применённое обновление — это её собственный смысл, у неё
есть публичный аксессор `versionOf()` и свои тесты. Изменилось только то, что
адаптер больше не выдаёт её за номер события верхушки: это были две разные
...
### Почему нельзя через `OutcomePrice`

`OutcomePrice` — цена outcome-токена рынка предсказаний, её инвариант — диапазон
