...
идентификатор сделки. В `last_trade_price` его НЕТ: `transactionHash`
опционален и не уникален на сделку (одна транзакция способна закрыть
несколько мэтчей). Собрать id из `tokenId + timestamp` значило бы выдумать
идентичность и получить коллизии в ленте.

Поэтому границей остаётся `TRADE_RECEIVED` с canonical VO — событие
наблюдения, которому идентичность сделки не нужна; различает наблюдения
canonical identity СООБЩЕНИЯ.
