...
открытия подписки (recorder-first). Сам координатор удалён после квалификации
нового коллектора; ниже — почему его схема не годилась. Координатор владел
источником, знал набор
...
### Кто переводит и кто подхватывает

Границу держит таймер сессии, а Gamma-резолюцией владеет `MarketFinalizer`.
Это разные компоненты и разные моменты времени, поэтому инвариант
формулируется по РЕЗУЛЬТАТУ:
...
`beginFinalization` устроен «ровно один раз» и второму вызывающему отвечает
`undefined`, поэтому у финализатора два источника снимка:
...
Дедупликация — по `_pending` финализатора. Момент границы едет в снимке
(`finalizingSinceMs`) и попадает в `finalization.startedAtMs` архива: брать
`now` в момент подхвата значило бы записать момент, который к границе
датасета отношения не имеет.

### Снос сироты откаченного приобретения

Снимок удерживаемого рынка доступен уже в `OPENING` (иначе терялся бы первый
book), и платой за это служит окно, в котором запись началась, а транзакция
ещё может откатиться:

```text
OPENING: subscribeMarket() ok → initial book → допуск → сессия + первая строка
         RTDS subscribe FAILED → rollback → claim исчез
         recording-сессия ОСТАЛАСЬ
```

На следующем тике приобретение повторится, но сессия уже существует — новый
book попал бы в тот же файл БЕЗ повторного допуска, и один датасет склеил бы
отказавшее поколение подписки, дыру и поколение-повтор. Поэтому
`runOnce()` начинается со сноса: `ACTIVE`-сессия со строками, не принятая
lifecycle и без claim-а, финализируется как `SHUTDOWN` (storage удаляет
незавершённый `.jsonl`).

«Нет claim-а» — надёжный признак, а не гонка: claim создаётся синхронно при
резервации рынка, до первого `await` транзакции, и исчезает только при откате
либо явном `release`.
