...
Переход `ACTIVE → FINALIZING` совершает тот, кто первым дошёл до границы, а
подхватывает его `MarketFinalizer` на следующем проходе — через
`getFinalizingSession()`. Инвариант формулируется по результату, а не по
инициатору: кто бы ни перевёл рынок, финализатор получает ровно один
immutable снимок и регистрирует рынок один раз.

`runOnce()` начинается со сноса СИРОТ: recording-сессия с записанными
строками, но без claim-а `collector:raw` — это откаченное приобретение
(RTDS-подписка отказала уже после первого book). Она удаляется как
незавершённый датасет (`finalizeMarket(SHUTDOWN)`), чтобы retry начал чистую
новую сессию, а не дописал отказавшее поколение подписки в тот же файл.
