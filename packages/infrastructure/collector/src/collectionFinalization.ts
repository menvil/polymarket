 ...
 *
 * До cutover они принадлежали legacy-координатору сбора вместе с его
 * сборщиком header-а версии 1. Координатор удалён после квалификации нового
 * коллектора, а DTO его пережили — и не случайно: это ФОРМАТ артефакта,
 * который читают и writer (finalizer), и все читатели архивов. Формат живёт
 * дольше того, кто его когда-то писал, поэтому его место — в canonical
 * пакете collection-контура, и второго набора одинаковых DTO в репозитории
 * быть не должно.
 *
