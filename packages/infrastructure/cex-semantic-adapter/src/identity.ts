 ...
 *
 * Символ не нормализуется (регистр, разделители, суффикс расчётной валюты
 * `:USDT`, дата экспирации `-260327`) — он остаётся тем, чем его назвала
 * биржа. Любая «умная» нормализация здесь схлопывала бы разные контракты в
 * один инструмент, а это ровно то, что запрещено.
 *
 * @example
 * ```typescript
