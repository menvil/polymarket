...
export class RecentVenueTradeIds {
  ...
  /**
   * Забывает идентификатор, зарегистрированный ранее.
   *
   * @param venueTradeId - Идентификатор, который следует снять с учёта
   * @returns `true`, если идентификатор помнился и был снят
   *
   * @remarks
   * Нужен ровно для одного сценария: сделка зарегистрирована ДО публикации
   * (чтобы её дубликат не проскочил, пока публикация идёт), а публикация
   * не удалась. Оставить регистрацию значило бы навсегда пометить
   * НЕопубликованную сделку как виденную: её повторная выдача биржей была
   * бы отброшена как дубликат, и дедуп превратился бы в потерю данных.
   *
   * Позиция в очереди вытеснения НЕ переиспользуется: запись остаётся в
   * `_order` «пустышкой» и будет вытеснена штатно. Так проще и, главное,
   * безопаснее — ёмкость считается по `_seen`, поэтому окно остаётся
   * ограниченным в любом случае.
   *
   * @example
   * ```typescript
   * if (seen.registerIfNew(id)) {
   *   if (!(await publishTrade())) seen.forget(id);
   * }
   * ```
   */
  public forget(venueTradeId: string): boolean {
    return this._seen.delete(venueTradeId);
  }
}
