 ...
 *
 * Границы проверяются по ЮНИКОДНЫМ классам, а не по ASCII-диапазону
 * `[a-zA-Z0-9]`. Разница не теоретическая: с ASCII-границей любая
 * нелатинская буква считается «не буквой», то есть пунктуацией, и заголовок
 * вида `Биткоинup or down?` проходил бы как наша серия. Vendor не обязан
 * публиковать только латиницу, а односторонняя ошибка здесь — чужой рынок
 * в realtime-контуре.
 *
 * В класс входят три категории, и третья — не перестраховка:
 *
 * - `\p{L}` — буквы, `\p{N}` — цифры: собственно «слово»;
 * - `\p{M}` — комбинирующие знаки. Одна и та же строка в юникоде
 *   представима слитно (NFC, `й` = U+0439) и разложенно (NFD, `и` + U+0306),
 *   и площадка нормализацию не гарантирует. В разложенной форме последним
 *   символом слова оказывается комбинирующий знак, а не буква: без `\p{M}`
 *   он читался бы как пунктуация, и `Бой̆up or down?` в NFD снова
 *   проходил бы как наша серия — ровно тот дефект, что уже чинился для
 *   нелатинских букв, просто уровнем ниже.
 */
const UP_OR_DOWN_PHRASE = /(?<![\p{L}\p{N}\p{M}])up\s+or\s+down(?![\p{L}\p{N}\p{M}])/iu;
