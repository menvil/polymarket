 ...
 * ```text
 * spec изменилась
 *   → await old.close()          ← старое поколение теряет право публиковать
 *   → освободить identity пула
 *   → factory(new config)
 *   → new.start()
 * ```
 *
 * Нужный инвариант — «не должно быть двух ПУБЛИКУЮЩИХ поколений», а не
 * «vendor-сокет физически уничтожен». Резолв `CexSource.close()` означает,
 * что supervised-петли источника завершены и инициировать `publish` ему
 * больше нечем; vendor при этом может дочищать соединение в фоне, и это
 * безопасно — «живой, но немой» сокет наблюдений не порождает.
 *
 * Требовать физического уничтожения было бы СИЛЬНЕЕ нужного и вредно:
 * зависший `instance.close()` останавливал бы реконсиляцию навсегда, а
 * из-за сериализации проходов — вместе с ней и все остальные биржи. Один
 * залипший vendor не должен убивать control-plane целиком.
 *
 * Пока `close()` не вернулся успешно, ключ пула остаётся занятым — включая
 * случай, когда `close()` ОТКАЗАЛ: тогда неизвестно, остановился ли
 * публикующий цикл, и поднимать поверх него новое поколение нельзя.
 *
...
export class CexSubscriptionController {
  ...
  public async close(): Promise<void> {
    ...
    this._closePromise = (async () => {
      ...
      for (const key of [...this._pools.keys()].sort(compareStrings)) {
        ...
        // Сначала остановка источника, только потом освобождение записи.
  ...
  private async _transition(
    ...
    if (desired === undefined) {
      ...
      // Ключ остаётся занятым, пока остановка не подтверждена: освободить
      // его раньше значило бы объявить identity свободной при, возможно,
      // ещё публикующем источнике.
    ...
    if (current !== undefined) {
      ...
      // Замена поколения: старое сперва теряет право публиковать, и только
      // потом появляется новое. Порядок буквальный: close → release
      // identity → factory → start. Перекрытие дало бы два ПУБЛИКУЮЩИХ
      // поколения одной routing identity (см. TSDoc класса).
      const closeFailure = await this._closeSource(
        current,
        healthy ? 'pool spec changed' : 'pool source terminal',
      );
      if (closeFailure !== null) {
        // Остановка не подтверждена: старое поколение могло не перестать
        // публиковать. Это проблема доступности пула, но не разрешение
        // поднять рядом второго публикующего — поколение остаётся за
        // контроллером.
   ...
   * @internal
   * @remarks
   * Ожидание обязательно: `start()` мог успеть поднять часть потоков, и
   * вернуть управление раньше их остановки значило бы разрешить
   * следующему проходу поднять поколение той же identity рядом с ещё
   * публикующим.
   *
   ...
   * остановка которого подтверждена. Проходы сериализованы, и подменить
   ...
   * @internal
   * @remarks
   * Успешный резолв `CexSource.close()` означает: источник больше никогда
   * не инициирует публикацию. Именно на этом держится запрет перекрытия
   * поколений — и именно поэтому ожидание здесь КОНЕЧНО: `close()` не
   * ждёт фоновой дочистки vendor-соединения, иначе одна залипшая биржа
   * подвесила бы реконсиляцию всех остальных.
   *
   * Отказ `close()` пул НЕ освобождает: неизвестно, остановился ли
   * публикующий цикл, а значит новое поколение той же identity могло бы
   * начать публиковать параллельно старому. Освобождение записи — работа
