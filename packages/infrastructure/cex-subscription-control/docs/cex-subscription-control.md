...
### Что именно запрещено: два ПУБЛИКУЮЩИХ поколения

Инвариант формулируется через публикации, а не через сокеты:

```text
await CexSource.close()
  ⇒ источник больше НИКОГДА не инициирует bus.publish()
  ⇒ новое поколение может стартовать
```

Гарантия держится на двух фактах транспорта: `RestartingTask.stop()` не просто
шлёт abort, а дожидается полного выхода петли сессий, а единственная точка
выхода в data plane — `_publish()`, который первым делом проверяет
`signal.aborted`. После завершения петли публиковать некому.

Vendor-сокет при этом может дочищаться в фоне, и это безопасно: дубли в шине
порождает публикующий цикл, а не живой сокет. «Живой, но немой» сокет
наблюдений не создаёт.

### Почему НЕ требуется физическое уничтожение сокета

Соблазнительно потребовать «старый транспорт уничтожен до старта нового». Это
сильнее нужного и вредно:
...
CexSource.close()     → снимает право публиковать; фоновой дочистки НЕ ждёт
```

Безусловное ожидание превратило бы зависший `instance.close()` в бессрочную
остановку `close()`, а через него — `reconcile()`, а через сериализацию
проходов — всего CEX control-plane: одна залипшая биржа останавливала бы
реконсиляцию всех остальных. Причём защищает от такого зависания ровно тот же
`closeTimeoutMs`, поэтому распространять защиту на границу жизненного цикла
обязательно.

Зафиксировано тестами: `зависший vendor-close не мешает поколению 2
стартовать, а поколение 1 больше не публикует` и `зависший vendor-close не
останавливает последующие проходы`
(`__tests__/real-source-integration.test.ts`, реальный `CexSource`).
Незавершённая дочистка при этом наблюдаема — её счётчик пишется в лог
закрытия источника.

### Identity не освобождается раньше подтверждённой остановки

Ключ пула остаётся занятым, пока `close()` не вернулся успешно — включая
случай, когда он **отказал**:

```text
close failure → неизвестно, остановился ли публикующий цикл
              → НЕ разрешение поднять рядом второго публикующего
```
...
которые он успел поднять), останавливает все пулы и очищает логическое
состояние. Ожидание конечно и при зависшем vendor-закрытии. Идемпотентен. Проход, стоявший в очереди на момент `close()`, отклоняется
