 ...
 * ```
 *
 * Идентичность рынка в этих событиях — ПАРА `venueId + marketId`, потому что
 * `MarketId` уникален только внутри пространства имён своей площадки (то же
 * правило, что у `Market.equals()` и ключа `MarketUniverse`). События, несущие
 * весь `Market`, дают её через сам рынок; события идентичности — двумя полями
 * payload.
 *
