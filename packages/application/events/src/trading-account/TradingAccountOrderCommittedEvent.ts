 ...
 * материализован.
 *
 * ### Обязанность producer'а: слить драфты доменных событий
 *
 * `Order` копит драфты `OrderEvent` во внутреннем буфере при каждом переходе
 * (`create`, `accept`, `applyFill`, …), а `pullEvents()` опустошает его
 * МУТАЦИЕЙ. Поэтому producer обязан вызвать `pullEvents()` ДО публикации:
 *
 * ```typescript
 * const accepted = order.accept();
 * const domainEvents = accepted.pullEvents(() => generator.nextRoot());
 * // теперь заявку можно класть в payload
 * ```
 *
 * Иначе в приватное состояние попадёт заявка с неслитым буфером, и любой, кто
 * позже вызовет на ней `pullEvents()`, мутирует объект, лежащий в состоянии.
 *
 * Проверить это на стороне потребителя нельзя: буфер не имеет публичного
 * read-only доступа — единственный способ его прочитать опустошает. Поэтому
 * контракт держится соглашением, а не проверкой.
 *
 * Что делать с полученными `OrderEvent` — дело producer'а: приватное
 * состояние на них не подписано (см. `AccountStateProjector`), потому что они
 * не несут `Portfolio`.
 *
 * ### Producer
 *
