...
/**
 * Контракт неизменяемости снимка (обязателен для реализаций порта).
 *
 * @remarks
 * `readonly` в типах — только compile-time обещание: оно ничего не значит
 * для потребителя, пришедшего из JS, и не мешает `as`-касту. Снимок при
 * этом ЯВЛЯЕТСЯ внутренним состоянием реализации: `getSnapshot()` отдаёт
 * последний успешный обход, а не копию на каждый вызов.
 *
 * Поэтому реализация обязана заморозить (`Object.freeze`) сам снимок,
 * массив `entries`, каждую запись и её `metrics` — иначе строка вида
 * `snapshot.entries[0].metrics.liquidity = x` у любого потребителя молча
 * меняет source of truth discovery. Заморозка выполняется на СОБСТВЕННЫХ
 * объектах реализации; чужие объекты, переданные ей извне, замораживать
 * нельзя — это побочный эффект на данных вызывающего.
 *
 * `market` не замораживается: `Market` — иммутабельная доменная entity,
 * и её неизменяемость — контракт `@polymarket/market`, а не этого порта.
 */
