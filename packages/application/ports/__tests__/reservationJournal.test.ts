...
describe('reservationJournal', () => {
  ...
  describe('heldReservation', () => {
    it('initial > 0 → HELD, remaining === initial', () => {
      expect(heldReservation('USDC', '65')).toEqual({
        kind: 'USDC', initial: '65', remaining: '65', consumed: '0', released: '0', status: 'HELD',
      });
    });

    it('initial === 0 → SETTLED', () => {
      expect(heldReservation('TOKENS', '0').status).toBe('SETTLED');
    });

    it('отрицательный initial → бросает NEGATIVE_AMOUNT', () => {
      expect(() => heldReservation('USDC', '-1')).toThrow(ReservationTransitionError);
    });
  });

  describe('hasHeldReservation', () => {
    it('HELD с remaining > 0 → true', () => {
      expect(hasHeldReservation(heldReservation('USDC', '10'))).toBe(true);
    });
    it('NONE → false', () => {
      expect(hasHeldReservation(emptyReservation('USDC'))).toBe(false);
    });
    it('SETTLED → false', () => {
      const settled = applyReservationDelta(heldReservation('USDC', '10'), { consume: '10' });
      expect(settled.ok && hasHeldReservation(settled.value)).toBe(false);
    });
    it('PARTIALLY_SETTLED с remaining > 0 → true', () => {
      const partial = applyReservationDelta(heldReservation('USDC', '10'), { consume: '4' });
      expect(partial.ok && hasHeldReservation(partial.value)).toBe(true);
    });
    it('RECONCILIATION_REQUIRED с remaining > 0 → true', () => {
      const rec = applyReservationDelta(heldReservation('USDC', '10'), { status: 'RECONCILIATION_REQUIRED' });
  ...
  describe('canConsumeHeldReservation', () => {
    it('HELD с remaining > 0 → true', () => {
      expect(canConsumeHeldReservation(heldReservation('USDC', '10'))).toBe(true);
    });
    it('PARTIALLY_SETTLED с remaining > 0 → true', () => {
      const partial = applyReservationDelta(heldReservation('USDC', '10'), { consume: '4' });
  ...
  describe('applyReservationDelta', () => {
    it('consume уменьшает remaining, увеличивает consumed, сохраняет инвариант', () => {
      const r = applyReservationDelta(heldReservation('USDC', '65'), { consume: '30' });
    ...
    it('exact decimal (без float-погрешности)', () => {
      const r = applyReservationDelta(heldReservation('USDC', '0.3'), { consume: '0.1' });
  ...
  describe('запрещённые состояния статус↔суммы (1.5)', () => {
    it('явный SETTLED при remaining > 0 → INVARIANT_VIOLATION', () => {
      const r = applyReservationDelta(heldReservation('USDC', '10'), { consume: '4', status: 'SETTLED' });
      expect(r.ok).toBe(false);
      if (!r.ok) expect(r.error.code).toBe('INVARIANT_VIOLATION');
    });

    it('явный HELD при remaining === 0 → INVARIANT_VIOLATION', () => {
      const r = applyReservationDelta(heldReservation('USDC', '10'), { consume: '10', status: 'HELD' });
      expect(r.ok).toBe(false);
      if (!r.ok) expect(r.error.code).toBe('INVARIANT_VIOLATION');
    });

    it('явный PARTIALLY_SETTLED при remaining === 0 → INVARIANT_VIOLATION', () => {
      const r = applyReservationDelta(heldReservation('USDC', '10'), { consume: '10', status: 'PARTIALLY_SETTLED' });
      expect(r.ok).toBe(false);
      if (!r.ok) expect(r.error.code).toBe('INVARIANT_VIOLATION');
    });

    it('явный NONE при ненулевых суммах → INVARIANT_VIOLATION', () => {
      const r = applyReservationDelta(heldReservation('USDC', '10'), { consume: '4', status: 'NONE' });
      expect(r.ok).toBe(false);
      if (!r.ok) expect(r.error.code).toBe('INVARIANT_VIOLATION');
    });
