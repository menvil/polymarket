...
| слово в `required` и `excluded` одновременно | «есть и нет» — невыполнимо для любого текста |
| `excluded` покрывает весь `anyOf` | выполнимых альтернатив не остаётся |
| вид рынка CEX вне словаря | граница транспорта такую policy не отобразит |
...
Противоречие слов ищется по **юникодной эквивалентности самого матчера**, а
не по собственной нормализации: `S` и `ſ` регекс под `iu` считает одним
словом, а `toLowerCase()` — разными, и такая policy проходила бы валидацию
будучи невыполнимой. Подробности — в [`market-filter.md`](./market-filter.md).

Словарь видов рынка CEX выведен из объекта под
`satisfies Record<CexPolicyMarketType, true>`, поэтому добавление четвёртого
вида — **ошибка компиляции**, а не молчаливо неполный список. Прежняя
аннотация `readonly CexPolicyMarketType[]` этого не давала: массив из двух
членов трёхчленного union-а — всё ещё корректный массив.
...
- [`policy-config.md`](./policy-config.md) — граница «plain config → Policy»
