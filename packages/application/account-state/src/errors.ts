 ...
 * исполнения, обе версии и `lastMutationAt` остаются нетронутыми.
 */
 ...
 *
 * @remarks
 * Навигация приватного состояния адресует записи по `InstrumentId` — тем же
 * идентификатором, которым адресуется рынок в торговом состоянии. Актив,
 * который в `InstrumentId` не превращается, нельзя связать с рыночным
 * контекстом: по такой заявке нельзя ни найти стакан, ни оценить позицию.
 *
