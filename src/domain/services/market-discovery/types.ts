...
export interface GammaMarketData {
  ...
  /** Best ask price */
  bestAsk?: number;
  /** Order minimum size (shares) - from Gamma API */
  orderMinSize?: number;
  /** Order price minimum tick size - from Gamma API */
...
export interface OutcomeCandidate {
  ...
  tokenId: string;
  /** Human-readable outcome name (e.g., "Up", "Down", "Up", "Down") */
  name: string;
}
...
export interface MarketCandidate {
  ...
  /** Market question */
  question: string;
  /** Market slug (for API calls - КРИТИЧНО для getMarketConstraints!) */
  slug?: string;
