...
export class FairValueCalculator {
  ...
  /**
   * Calculates UP/DOWN mismatch percentage
   *
   * @param fairUp - Fair value of UP token
   * @param fairDown - Fair value of DOWN token
   * @returns Percentage mismatch (should be close to 0%)
   *
   * @remarks
   * В идеале: fairUp + fairDown = 1.0
   ...
   * Формула:
   * ```
   * mismatch = |fairUp + fairDown - 1.0| / 1.0 * 100%
   * ```
   *
   * @example
   * ```typescript
   * const calculator = new FairValueCalculator();
   *
   * const fairUp = Price.fromNumber(0.65);
   * const fairDown = Price.fromNumber(0.34);
   *
   * const mismatch = calculator.calculateUpDownMismatch(fairUp, fairDown);
   * console.log(`Mismatch: ${mismatch.value}%`); // 1%
   *
   * if (mismatch.value > 1.0) {
   *   console.warn('Significant UP/DOWN mismatch detected!');
   * }
   * ```
   */
  public calculateUpDownMismatch(fairUp: Price, fairDown: Price): Percentage {
    const sum = fairUp.value + fairDown.value;
