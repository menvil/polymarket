 ...
 * Hedge Ratio:
 * ```
 * hedgeRatio = min(yesQuantity, noQuantity) / max(yesQuantity, noQuantity)
 * ```
 *
 * Интерпретация:
...
export class HedgeRatioCalculator {
   ...
   *
   * @param yesQuantity - YES position quantity
   * @param noQuantity - NO position quantity
  ...
  public calculate(
    ...
    const yesQty = yesQuantity.value;
    const noQty = noQuantity.value;
  ...
  public calculateForMarket(
    ...
    const yesQuantity = yesPosition
      ? yesPosition.totalQuantity
      : Quantity.fromNumber(0);
    const noQuantity = noPosition
      ? noPosition.totalQuantity
      : Quantity.fromNumber(0);

    return this.calculate(yesQuantity, noQuantity, targetRatio);
  }

  /**
   * Calculates hedge effectiveness
   *
   * @param yesQuantity - YES quantity
   * @param noQuantity - NO quantity
  ...
  public calculateEffectiveness(
    ...
    const result = this.calculate(yesQuantity, noQuantity);
