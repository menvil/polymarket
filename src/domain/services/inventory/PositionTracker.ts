 ...
 * Формулы:
 * ```
 * netPosition = upQuantity - downQuantity
 * grossPosition = |upQuantity| + |downQuantity|
...
export class PositionTracker {
  ...
  public calculateMetrics(portfolio: Portfolio, marketId: string): PositionMetrics {
    ...
    const netPosition = upQuantity - downQuantity;

    // Calculate gross position (absolute)
    const grossPosition = Math.abs(upQuantity) + Math.abs(downQuantity);

    // Calculate hedge ratio
    let hedgeRatio = 0;
    if (upQuantity > 0 || downQuantity > 0) {
      const minQty = Math.min(upQuantity, downQuantity);
      const maxQty = Math.max(upQuantity, downQuantity);
