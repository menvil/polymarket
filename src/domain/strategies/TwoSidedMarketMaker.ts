 ...
 *
 * @remarks
 * **LEGACY STRATEGY** - используется только в ORCHESTRATOR mode.
 * Для нового multi-strategy mode используйте DumbStrategy.
 *
 * ⚠️ TODO (v5): Исправить Date.now() usage (lines 425, 589)
 * - Нужно передавать currentTime через параметр или ctx
 * - Текущее использование нарушает детерминизм replay
 * - НЕ критично пока используется только в legacy mode
 *
