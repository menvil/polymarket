...
export interface IExecutionAdapter {
   ...
   *
   * @param order - Ордер для размещения (содержит strategyId с v4.2)
   ...
   * 6. Публикует OrderAccepted event с strategyId (v4.2)
   *
   * v4.2 (Фаза 4): strategyId для multi-strategy изоляции
   * - strategyId включен в Order entity
   * - strategyId propagated через: Intent → Order → Adapter → Event
   * - Optional для обратной совместимости
   *
   * @example
   * ```typescript
