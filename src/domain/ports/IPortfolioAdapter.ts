...
export interface IPortfolioAdapter {
   ...
   *
   * NOTE: Locked баланс можно получить через getLockedBalance() (если нужно)
   *
   * @example
   * ```typescript
   ...
   * const position = await portfolioAdapter.getPosition(yesTokenId);
   ...
   *
   * NOTE: Для real-time updates используйте WebSocket подписку отдельно
   *
   * @example
   * ```typescript
