 ...
 * const noTokenId = market.outcomes[1].tokenId;
 *
 * dataFeed.subscribeToOrderbook(yesTokenId, (ob) => {
 *   console.log('YES orderbook updated');
 * });
 *
 * dataFeed.subscribeToOrderbook(noTokenId, (ob) => {
...
export interface IMarketDataFeed {
   ...
   *   подписка происходит через subscribeToMarket(yesTokenId, noTokenId)
   */
   ...
   * - **УСТАРЕЛО**: Используйте unsubscribeFromMarket(yesTokenId, noTokenId)
   */
