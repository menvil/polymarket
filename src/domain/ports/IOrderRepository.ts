...
export interface IOrderRepository {
   ...
   *
   * @param tokenId - ID токена (UP или DOWN)
  ...
  /**
   * Находит все ордера для рынка (по обоим токенам UP и DOWN)
   *
   * @param upTokenId - ID токена UP
   * @param downTokenId - ID токена DOWN
   * @returns Массив ордеров для обоих токенов
   *
   * @remarks
   * - Объединяет результаты для UP и DOWN токенов
   * - Правильный способ найти все ордера маркета
   */
  findByMarketTokens(upTokenId: string, downTokenId: string): Promise<Order[]>;
