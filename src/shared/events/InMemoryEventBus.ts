/**
 * InMemoryEventBus - Асинхронная шина событий в памяти
 *
 * @remarks
 * Минималистичная реализация IEventBus для in-memory pub/sub.
 * Реализует контракт IEventBus без дополнительной бизнес-логики.
 *
 * ## Реализация (implementation details):
 *
 * ### 1. Async boundary через setImmediate
 ...
 *
 * ### 2. Error isolation через try/catch
 ...
 *
 * ### 3. FIFO очередь (НЕ контракт!)
 * - Текущая реализация использует **FIFO очередь** (queue + drain)
 * - События обрабатываются в порядке добавления в очередь
 * - **ВАЖНО**: Это implementation detail, НЕ гарантия контракта!
 * - Subscribers НЕ должны полагаться на FIFO порядок
 *
 ...
 * - EventBus делает **одну попытку** вызвать каждый handler
 * - Subscribers **должны быть idempotent** (для будущих retry механизмов)
 *
 * @example
 * ```typescript
 * import { ConsoleLogger } from './infrastructure/logging/ConsoleLogger';
 *
 * // Создание с default error handler
 * const logger = new ConsoleLogger();
 * const bus = new InMemoryEventBus(logger);
 *
 * // Или с custom error handler
 ...
 *   console.log('Orderbook:', event.assetId);
 * });
 *
 ...
 * // → handler вызовется ПОЗЖЕ (асинхронно, порядок - implementation detail)
 *
 * // Отписка
 * unsubscribe();
 * ```
...
/**
 * InMemoryEventBus - Реализация асинхронной шины событий
 *
 * @remarks
...
export class InMemoryEventBus implements IEventBus {
  ...
  /**
   * Очередь событий для асинхронной доставки
   *
   * @remarks
   * События добавляются через `publish()` и обрабатываются через `drain()`.
   * FIFO (First In First Out) порядок гарантирован.
   */
   ...
   *
   * Алгоритм:
   * 1. Добавить событие в очередь (FIFO)
   ...
   * // → handlers вызовутся ПОЗЖЕ (в следующем event loop tick)
   * ```
   */
  ...
  /**
   * Доставляет событие всем subscribers
   *
   * @param event - Событие для доставки
   *
   * @remarks
   * Алгоритм текущей реализации:
   * 1. Вызвать specific event handlers (для event.eventName)
  ...
  private deliverEvent(event: DomainEvent): void {
    ...
    if (specificHandlers) {
      for (const handler of specificHandlers) {
        this.invokeHandler(handler, event, 'specific');
      }
    }

    // 2. Вызвать "all events" handlers
    for (const handler of this.allHandlers) {
      this.invokeHandler(handler, event, 'all');
    }
  }

  /**
   * Вызывает handler с изоляцией ошибок
   *
   * @param handler - Handler для вызова
   * @param event - Событие для передачи в handler
  ...
  private invokeHandler(
    ...
    try {
      ...
      try {
        this.errorHandler.handle(error, event, handlerType);
        ...
        console.error('[EventBus] Event:', event.eventName, event.eventId);
      }
    }
  }
