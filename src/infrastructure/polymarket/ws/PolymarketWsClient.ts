...
import type { WebSocketManager, SubscribeParams, ConnectionStatus } from '../../exchange/clients/WebSocketManager.js';
...
export class PolymarketWsClient extends EventEmitter {
   ...
   *
   * @param wsManager - Экземпляр WebSocketManager для обёртывания
   ...
   * @example
   * ```typescript
   * const wsManager = new WebSocketManager({ url: '...', logger });
   * const client = new PolymarketWsClient(wsManager, logger);
   * ```
   */
