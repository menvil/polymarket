 ...
 *
 * @deprecated **LEGACY MULTI-MARKET SYSTEM** - НЕ используется в production.
 *
 * ⚠️ ИСПОЛЬЗУЙТЕ ВМЕСТО ЭТОГО: MultiStrategyCoordinator
 *
 * Причины deprecation:
 * - Использует MainTradingOrchestrator (deprecated) для каждого рынка
 * - Жёстко связанная архитектура vs event-sourced стратегии
 * - Нет multi-strategy isolation (один orchestrator = один рынок)
 * - Нет replay capability vs детерминированный replay
 *
 * Новая система (v4.2):
 * - MultiStrategyCoordinator заменяет MultiMarketTrader
 * - DumbStrategy заменяет MainTradingOrchestrator
 * - Event-sourced архитектура с projectors
 *
 * TODO (v5): Удалить после полной миграции на DUMB_STRATEGY mode
 *
 * @remarks
...
export class MultiMarketTrader {
  ...
  async removeMarket(
    ...
    try {
      ...
      // For now we use zero PnL - in future we can add getPnL() to orchestrator
