...
На аутентифицированном пути считаются по нашей формуле из `feeRateBps`:
...
На публичном пути ставки нет, и колонка **FEES печатает `—`, а не `$0.00`**.
Ноль читался бы как «комиссий не было», тогда как правда — «величина не
отделена от `realizedPnl`».
...
Замечено на боевых данных: у элемента `makerOrders` ставка часто `null`,
хотя верхний уровень сделки отдаёт `"0"` — в этом случае действует ставка
самой сделки.
