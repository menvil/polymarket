...
Порог по умолчанию — **10 секунд**. RTDS идёт с частотой около 1 Гц, так что
10 с тишины — это десяток пропущенных тиков подряд: ложных срабатываний быть
не может (замеренный p99 доставки ≈ 0.4 с).

Было 30 с. Живой разрыв в run-02 показал, где настоящая цена:

```text
провал данных        ~34 с
из них обнаружение    32.5 с   ← 96 %
собственно reconnect   1.3 с   ←  4 %
```

Узкое место — порог, а не скорость восстановления. Снижение втрое стоит одной
константы и даёт больше, чем любое усложнение механики восстановления.
...
### Второй уровень: сброс общего соединения
...
Что в run-02 умерли **все шесть** RTDS-фидов одновременно — это подпись общего
соединения, а не шести независимых потоков.

Поэтому: если фид переоткрыли и он умер **снова, не приняв ни одного
события**, эскалируем на уровень транспорта.

```text
фид умер → reopen → умер снова без единого события (consecutiveRestarts >= 2)
  → client.closeSubscriptions()
  → итераторы ВСЕХ подписок завершаются
  → каждый надзорный цикл поднимает СВОЮ подписку из собственного reopen
```

Счётчик обнуляется первым же пришедшим событием, поэтому «подряд» означает
именно «переоткрыли, и он не ожил», а обычный сетевой всплеск до эскалации не
доходит.

`closeSubscriptions()` по контракту SDK — _«ends active subscription iterators
and closes shared websocket connections… does not affect authentication or
other client state»_, то есть клиент после него переиспользуем.

**Кэш подписок — это сами живые циклы.** Отдельного реестра нет: у каждой
подписки её spec живёт в замыкании `reopen`, и множество работающих циклов и
есть кэш. Ровно так это было устроено в legacy `RtdsWebSocketClient` («кэш
подписок для восстановления после reconnect»), только там реестр был явным.

**Single-flight и cooldown обязательны**, потому что фиды умирают пачкой: без
них каждый из шести потребовал бы своего сброса, и мы бы рвали соединение
шесть раз подряд, не давая ему встать. Идущий сброс переиспользуется,
следующий возможен не раньше 30 с.
...
завершился сам, физическая подписка исчезла, а контроллер продолжает считать
рынок ACTIVE.

Поэтому watchdog CLOB не получает, а `reopen` — получает. Переоткрытие
ТЕХ ЖЕ `tokenIds` владения рынками не меняет: это восстановление той же самой
подписки, а не решение собирать новый рынок.

Итоговая таблица семантик:

| поток | тишина | конец итератора | исключение итератора |
| --- | --- | --- | --- |
| RTDS (watched) | перезапуск | перезапуск | перезапуск |
| CLOB | норма | перезапуск | перезапуск |

Терминальный отказ источника остался ровно за тем, что восстановить нельзя, —
**отказом шины**. Раньше конец CLOB-итератора тоже ронял источник; это делало
смерть видимой, но невосстановимой, а пересборку источника composition root
не выполняет (её TSDoc относит эту работу к нему, но кода нет). Видимая и
невосстановимая смерть — плохой размен, если восстановление стоит одного
замыкания.
...
interface PolymarketSubscriptionHealth {
  ...
  readonly watched: boolean;       // тишина здесь = отказ (RTDS), а не норма (CLOB)
  readonly broken: boolean;        // лестница пройдена, ретраи продолжаются
}
```

`watched` обязателен к учёту потребителем: с тех пор как CLOB тоже попал в
снимок, агрегат «сколько молчит самый тихий поток» обязан считаться **только
по watched**, иначе спокойный рынок выглядел бы как авария. В статусе
коллектора это `pmRtdsSilentSec` (только watched) против `pmFeedRestarts` /
`pmFeedBroken` (по всем подпискам) и `pmConnResets`.
