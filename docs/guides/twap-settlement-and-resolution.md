...
flowchart TD
    ...
    POLL --> OFF{"официальные данные<br/>дают победителя?"}
    OFF -->|да| OC["OFFICIAL COMPLETE<br/>provenance = official"]
    OFF -->|нет| T{"бюджет 60 мин исчерпан<br/>ИЛИ shutdown?"}
    T -->|нет| POLL
    T -->|да| FB{"deterministic TWAP<br/>fallback возможен?"}
...
### Условие завершения — «победитель известен», а не «есть оба числа»

Live-наблюдение 2026-08-26: рынок получил `uma=resolved` и цены `1/0` на
4.5-й минуте, а `finalPrice` не публиковался ещё долго. Прежнее условие
(«priceToBeat И finalPrice») держало бы такой рынок все 60 минут и
заархивировало бы его как `timeout` при уже известном официальном итоге.

Недостающие числа при этом не теряются: они восполняются из записанного
потока и помечаются `provenance.priceToBeat`/`finalPrice = 'derived'`.
Derived-значение никогда не выдаётся за официальное.
