...
| Прогон                                            | Осталось после остановки |
| ------------------------------------------------- | ------------------------ |
| 5 мин, 100 рынков, 6 бирж                          | 0 сокетов, выход сам      |
| 5 мин, принудительные плановые рестарты CCXT (45 c) | 0 сокетов, выход сам     |
| checkpoint full 18 мин, 3 архива                   | `{FSReqCallback: 1}`, exit 0 |

До остановки в этих прогонах живы ровно ожидаемые сокеты — по два на биржу,
`ws-subscriptions-clob`, `ws-live-data` и (в момент опроса) `gamma-api`; после
остановки не остаётся ни одного.

Важно зафиксировать честно: **исходное «зависание» наблюдалось однократно** — на
прогоне 31.6 минуты с девятью архивами и десятиминутным дренажом. Первичное
объяснение («keep-alive пул Gamma, который нечем закрыть») оказалось НЕВЕРНЫМ:
метод закрытия у клиента есть, а сокеты `gamma-api` в замерах освобождаются сами.
Воспроизвести зависание на более коротких прогонах не удалось, поэтому
`closeSubscriptions()` добавлен как закрытие реального разрыва во владении, а не как
доказанное лекарство от того конкретного случая.
