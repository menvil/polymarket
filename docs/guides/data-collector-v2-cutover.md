...
Операционное следствие: под pm2 процесс завершает финальный `SIGKILL` по
`kill_timeout`. Это штатно — к тому моменту вся работа сделана, — но таймаут обязан
превышать длительность самой остановки. На production-масштабе (100 рынков, 6 бирж,
~1.4M записей) от `SIGTERM` до «Shutdown complete» замерено **79 секунд**, поэтому
прежних 30 секунд не хватало: SIGKILL приходил посреди финализации и терял текущие
окна записи. `kill_timeout` поднят до 180 секунд.
