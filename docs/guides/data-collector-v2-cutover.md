...
> **Статус:** `@polymarket/collection-coordinator`, описанный ниже как legacy
> behavior oracle, **удалён из репозитория** после успешной квалификации
> нового коллектора. Документ сохранён как история миграции.
...
Официальный SDK-клиент создаётся ОДИН НА КОМПОЗИЦИЮ и разделяется тремя её
компонентами:
...
Границей владения служит именно композиция, а не процесс: каждый вызов
`createDataCollector` без явного `options.client` создаёт СВОЙ клиент. В
production это одно и то же (процесс поднимает один контур), но короткий режим
checkpoint поднимает две композиции подряд — и клиентов там два, каждый со своим
закрытием. Передать общий клиент снаружи можно тем же способом, что и bus:
