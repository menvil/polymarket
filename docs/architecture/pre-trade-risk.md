...
> **LEGACY / HISTORICAL**
>
> Пакет `@polymarket/risk` перенесён в
> `legacy-bot/trading-contour-reference/` вместе со всем старым торговым
> контуром: он был мёртв — ноль импортов из живого дерева. Код там не
> собирается и не запускается.
>
> Документ сохранён ради решений: какие проверки понадобились пре-трейд
> гейту и почему они устроены fail-closed. Новый контур будет строить риск
> заново, поверх canonical event pipeline.
