...
const statusInterval = setInterval(() => {
  ...
  logger.info('Collector status', {
    ...
    ignoredNotHeld: status.gate.ignoredNotHeldByCollector,
    // Жизненный цикл записей: сколько пишется, сколько за границей датасета.
    activeSessions: status.collection.activeSessions,
    finalizingSessions: status.collection.finalizingSessions,
    sealed: status.collection.sealedTotal,
    claimsReleased: status.collection.claimsReleased,
    orphansDiscarded: status.collection.orphanSessionsDiscarded,
    lifecycleFailures: status.collection.finalizationFailures,
    // Резолюция: чем закончились истёкшие рынки.
    pendingFinalizations: status.finalization.pendingFinalizations,
    archived: status.finalization.archivedTotal,
    official: status.finalization.officialFinalizations,
    fallback: status.finalization.fallbackFinalizations,
    discarded: status.finalization.discardedUnresolvable,
    archiveFailures: status.finalization.archiveFailures,
