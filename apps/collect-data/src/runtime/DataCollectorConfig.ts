 ...
 * адресуемым тройкой `exchangeId + marketType + stream`, а несколько профилей одной биржи
 ...
 * записи `cex-config.json` и адресуются ТОЙ ЖЕ тройкой, которой контроллер
 * ключует физический пул, — `exchangeId + marketType + stream`: фабрика
 * источников накладывает их на тот пул, для которого поднимает источник. Общих на все биржи значений здесь нет — это молча ломало бы
...
export interface CexExchangeConfig {
   ...
   * так его ключует сам контроллер), поэтому транспорт адресуется этой же
   * тройкой, а не одной биржей.
   */
