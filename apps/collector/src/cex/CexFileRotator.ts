 ...
 * rotator.start();
 * rotator.write('binance', 'BTC/USDT', { t: 'ob', ts: Date.now(), bids: [...], asks: [...] });
 * await rotator.close();
 * ```
 */
...
export class CexFileRotator {
   ...
   * @returns Путь: `outputDir/YYYY-MM-DD/{exchange}/{exchange}_{safeSymbol}_{YYYY-MonthName-DD}_{HMMam}-{HMMam}_ET.jsonl`
   *
   * @example
   * ```typescript
