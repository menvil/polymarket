/**
 * PairedCexCrowdStrategy — pair-aware торговля по 4D-таблице `(delta, tau, crowd, regime)`.
 *
 * @remarks
 * Идея:
 ...
 * @example
 * ```typescript
 * const strategy = new PairedCexCrowdStrategy({
 ...
 * Параметры стратегии.
 */
export interface PairedCexCrowdConfig {
  ...
  /** Не рекомендуется для live/paper: статический venue-basis быстро устаревает. */
  ...
  /** Логировать каждый фактический ENTRY в application logger (default true). */
  readonly logEntries?: boolean;
  /** Логировать все decision/reject/hold события с полным state (default false). */
  readonly logDecisions?: boolean;

  /** После какого возраста maker BUY считаем non-fill информативным (default 3000ms). */
  readonly noFillDecisionMs?: number;
  /** Минимальный drift в центах против стоящего BUY чтобы переключать логику (default 1.5¢). */
  readonly noFillDriftCents?: number;
  /** Минимальный edge для opportunistic opposite-side entry после non-fill (default 0.015). */
  readonly oppositeEntryMinEdge?: number;
  /** Максимальный tape imbalance по opposite side для opportunistic entry (default 0.15). */
  readonly oppositeMaxImbalance?: number;
  /** Штраф к edge в центах для той же стороны после adverse fill (default 0.5¢). */
  readonly fillAdversePenaltyCents?: number;
  /** Пауза перед повторным входом в ту же сторону после закрытия позиции (default 8000ms). */
  readonly sameSideReentryCooldownMs?: number;
  /** Минимальная зафиксированная прибыль на share для paired-lock выхода (default 0.25¢). */
  readonly pairedLockMinProfitCents?: number;
  /** Буфер на комиссии/ошибки модели для paired lock (default 1.0¢). */
  readonly pairedLockFeeBufferCents?: number;
}

interface TapeSignal {
  readonly imbalance: number;
  readonly vwapCents: number | undefined;
...
interface CCData {
  ...
  readonly primaryTape: TapeSignal;
  readonly complementaryTape: TapeSignal | undefined;
...
export class PairedCexCrowdStrategy extends BaseStrategy<CCData, CCAction> {
  ...
  private readonly _logDecisions: boolean;
  private readonly _noFillDecisionMs: number;
  private readonly _noFillDriftCents: number;
  private readonly _oppositeEntryMinEdge: number;
  private readonly _oppositeMaxImbalance: number;
  private readonly _fillAdversePenaltyCents: number;
  private readonly _sameSideReentryCooldownMs: number;
  private readonly _pairedLockMinProfitCents: number;
  private readonly _pairedLockFeeBufferCents: number;
  ...
  private _sameSideBlockedUntilMs = 0;
  private _lastExitedSide: EntrySide | undefined;
  ...
  constructor(
    ...
    this._logDecisions = config.logDecisions ?? false;
    this._noFillDecisionMs = config.noFillDecisionMs ?? 3_000;
    this._noFillDriftCents = config.noFillDriftCents ?? 1.5;
    this._oppositeEntryMinEdge = config.oppositeEntryMinEdge ?? 0.015;
    this._oppositeMaxImbalance = config.oppositeMaxImbalance ?? 0.15;
    this._fillAdversePenaltyCents = config.fillAdversePenaltyCents ?? 0.5;
    this._sameSideReentryCooldownMs = config.sameSideReentryCooldownMs ?? 8_000;
    this._pairedLockMinProfitCents = config.pairedLockMinProfitCents ?? 0.25;
    this._pairedLockFeeBufferCents = config.pairedLockFeeBufferCents ?? 1.0;

    this._logger?.warn('PairedCexCrowd: loaded', {
      ...
      logDecisions: this._logDecisions,
      ...
      noFill: {
        decisionMs: this._noFillDecisionMs,
        driftCents: this._noFillDriftCents,
        oppositeEntryMinEdge: this._oppositeEntryMinEdge,
        oppositeMaxImbalance: this._oppositeMaxImbalance,
      },
      pairedLock: {
        minProfitCents: this._pairedLockMinProfitCents,
        feeBufferCents: this._pairedLockFeeBufferCents,
      },
      exits: {
        mode: this._exitMode,
        ...
        fillAdversePenaltyCents: this._fillAdversePenaltyCents,
        sameSideReentryCooldownMs: this._sameSideReentryCooldownMs,
      },
    });
  }
  ...
  protected gather(snapshot: StrategySnapshot): CCData | undefined {
    ...
    const primaryTape = this._buildTapeSignal(snapshot.tradeTape, snapshot.nowMs);
    ...
    let complementaryTape: TapeSignal | undefined;
    if (compId !== undefined && compAsset !== undefined) {
      ...
      complementaryTape = this._buildTapeSignal(snapshot.complementaryTradeTape, snapshot.nowMs);
    }
    ...
    if (anyPosition) {
      ...
      if (this._inPosition && this._entrySide) {
        this._lastExitedSide = this._entrySide;
        this._sameSideBlockedUntilMs = snapshot.nowMs + this._sameSideReentryCooldownMs;
      }
  ...
  private _buildTapeSignal(
    tape: StrategySnapshot['tradeTape'] | StrategySnapshot['complementaryTradeTape'],
    nowMs: number,
  ): TapeSignal {
    if (!tape) {
      return { imbalance: 0, vwapCents: undefined, tradeCount: 0 };
    }
  ...
  protected decide(data: CCData): CCAction[] {
    ...
    if (this._inPosition && exitTarget) {
      const pairedLock = this._buildPairedLock(data);
      ...
      if (exitReason) {
        if (exitTarget.availableTokenQty.gt(0)) {
          ...
          this._logger?.warn('PairedCexCrowd: EXIT', {
    ...
    let entryTarget = this._resolveEntryTarget(data, zone);
    if (!entryTarget) {
      this._recordDecision(data, 'SKIP', 'zone-side-unsupported');
      return [{ type: 'STOP' }];
    }

    const switchedTarget = this._resolveNoFillEntryTarget(data, zone, entryTarget);
    if (switchedTarget) {
      entryTarget = switchedTarget;
    }
    if (this._isSameSideBlocked(entryTarget.side, data.nowMs)) {
      this._recordDecision(data, 'SKIP', 'same-side-reentry-cooldown');
      return [{ type: 'STOP' }];
    }
    ...
    const cancelIds: OrderId[] = entryTarget.openOrderIds.length > 0
      ? []
      : [
        ...data.primary.openOrderIds,
        ...(data.complementary?.openOrderIds ?? []),
      ].filter((id) => !entryTarget.openOrderIds.includes(id));
    ...
    if (this._logEntries) {
      this._logger?.warn('PairedCexCrowd: ENTRY', {
    ...
    if (cancelIds.length > 0) {
      actions.push({ type: 'CANCEL_OPEN', orderIds: cancelIds });
    }
    actions.push({
      type: 'POST_BID',
      bidCents: bid,
  ...
  private _isSameSideBlocked(side: EntrySide, nowMs: number): boolean {
    return this._lastExitedSide === side && nowMs < this._sameSideBlockedUntilMs;
  }

  private _resolveNoFillEntryTarget(data: CCData, zone: EdgeZone, current: SideData): SideData | undefined {
    if (!this._pendingEntrySide || this._pendingEntrySide !== current.side) return undefined;
    if ((data.nowMs - this._pendingEntryAtMs) < this._noFillDecisionMs) return undefined;
    if (current.openBuyAtCents === undefined) return undefined;

    const currentMid = current.bestBidCents !== undefined && current.bestAskCents !== undefined
      ? (current.bestBidCents + current.bestAskCents) / 2
      : undefined;
    if (currentMid === undefined) return undefined;

    const drift = currentMid - this._pendingEntryPriceCents;
    if (drift < this._noFillDriftCents) return undefined;

    const opposite = current.side === 'UP' ? data.complementary : data.primary;
    if (!opposite) return undefined;
    if (this._isSameSideBlocked(opposite.side, data.nowMs)) return undefined;

    const oppositeBid = this._computeEntryBidCents(opposite);
    if (oppositeBid === undefined) return undefined;

    const zoneReject = this._entryZoneRejectReason(zone, oppositeBid, opposite.side);
    if (zoneReject) return undefined;
    const cexReject = this._cexEntryRejectReason(data, opposite.side);
    if (cexReject) return undefined;

    const edgeHat = this._entryEdgeHat(zone, oppositeBid, opposite.side);
    if (edgeHat < this._oppositeEntryMinEdge) return undefined;

    const oppositeTape = opposite.side === 'UP' ? data.primaryTape : data.complementaryTape;
    const directionalImbalance = opposite.side === 'UP'
      ? (oppositeTape?.imbalance ?? 0)
      : -(oppositeTape?.imbalance ?? 0);
    if (directionalImbalance > this._oppositeMaxImbalance) return undefined;

    this._logDecisionEvent('PairedCexCrowd: no-fill switch', {
      from: current.side,
      to: opposite.side,
      pendingAgeMs: data.nowMs - this._pendingEntryAtMs,
      pendingBidCents: this._pendingEntryPriceCents,
      currentMidCents: round(currentMid, 3),
      driftCents: round(drift, 3),
      oppositeEdgeHat: round(edgeHat * 100, 3),
      directionalImbalance: round(directionalImbalance, 4),
      cexDirection: data.cexDirectionForToken,
      cexValueBps: data.cexSignal?.value,
    });

    return opposite;
  }

  private _buildPairedLock(data: CCData): CCAction[] | undefined {
    if (!this._inPosition || !this._entrySide) return undefined;
    ...
    const lockedProfitCents = this._lockedProfitCents(targetShares, hedgeSize, hedgePriceCents)
      - this._pairedLockFeeBufferCents;
    if (lockedProfitCents < this._pairedLockMinProfitCents) return undefined;

    if (hedgeTarget.openBuyAtCents === hedgePriceCents || hedgeTarget.hasMatchedBuy) return [];

    this._logger?.warn('PairedCexCrowd: LOCK', {
      entrySide: this._entrySide,
      hedgeSide: hedgeTarget.side,
      hedgePriceCents,
      hedgeSize: hedgeSize.toFixed(4),
      lockedProfitCents: round(lockedProfitCents, 4),
    });
  ...
  private _recordDecision(
    ...
    this._logDecisionEvent('PairedCexCrowd: decision', {
      action,
      rejectReason,
      bidPrice: order?.bidCents,
      askPrice: order?.askCents,
  ...
  private _serializeDecisionState(
    ...
    return {
      ...
      tape: {
        primary: {
          imbalance: round(data.primaryTape.imbalance, 6),
          vwapCents: data.primaryTape.vwapCents !== undefined ? round(data.primaryTape.vwapCents, 6) : undefined,
          tradeCount: data.primaryTape.tradeCount,
        },
        complementary: data.complementaryTape ? {
          imbalance: round(data.complementaryTape.imbalance, 6),
          vwapCents: data.complementaryTape.vwapCents !== undefined
            ? round(data.complementaryTape.vwapCents, 6)
            : undefined,
          tradeCount: data.complementaryTape.tradeCount,
        } : undefined,
      },
      pending: {
        entrySide: this._pendingEntrySide,
        entryAgeMs: this._pendingEntryAtMs > 0 ? data.nowMs - this._pendingEntryAtMs : undefined,
        entryPriceCents: this._pendingEntryPriceCents || undefined,
        exitSide: this._pendingExitSide,
        exitAgeMs: this._pendingExitAtMs > 0 ? data.nowMs - this._pendingExitAtMs : undefined,
        exitPriceCents: this._pendingExitPriceCents || undefined,
      },
      positionState: {
        inPosition: this._inPosition,
        entryPlacedAtMs: this._entryPlacedAtMs || undefined,
        entryPriceCents: this._entryPriceCents || undefined,
        entrySize: this._entrySize.gt(0) ? this._entrySize.toString() : undefined,
        lastExitedSide: this._lastExitedSide,
        sameSideBlockedUntilMs: this._sameSideBlockedUntilMs || undefined,
        sameSideCooldownRemainingMs: this._sameSideBlockedUntilMs > data.nowMs
          ? this._sameSideBlockedUntilMs - data.nowMs
          : 0,
      },
  ...
  private _logDecisionEvent(message: string, payload: Record<string, unknown>): void {
    if (this._logDecisions) {
      this._logger?.warn(message, payload);
      return;
    }
    this._logger?.debug(message, payload);
  }
  ...
  private _entryEdgeHat(zone: EdgeZone, entryBidCents: number, side: EntrySide): number {
    const pHat = side === 'UP' ? zone.train.pHat : 1 - zone.train.pHat;
    const rawEdge = pHat - entryBidCents / 100;
    if (this._lastExitedSide === side) {
      return rawEdge - this._fillAdversePenaltyCents / 100;
    }
    return rawEdge;
  }
  ...
  private _buildHedgeExit(data: CCData, exitReason: string): CCAction[] | undefined {
    ...
    this._logger?.warn('PairedCexCrowd: HEDGE', {
