 ...
 */
export interface CexLeadLagExitPolicyConfig {
 ...
 * @example
 * ```typescript
 * const strategy = new CexLeadLagExitPolicyStrategy(
...
export class CexLeadLagExitPolicyStrategy extends BaseStrategy<CexLeadLagData, CexLeadLagAction> {
   ...
   * @param strategyId - Уникальный идентификатор экземпляра. Default: "cex-lead-lag-exit-policy-1"
  ...
  constructor(
    ...
    if (config.exitPolicyTablePath) {
      ...
      this._logger?.warn('CexLeadLagExitPolicy: exit-policy table ENABLED', {
    ...
    this._logger?.warn('CexLeadLagExitPolicy: init (signal-first)', {
      strategyId: this.id,
      side: this._side,
      mode: this._mode,
  ...
  private _checkExitSignalFirst(
    ...
    const breakEvenTargetHit =
      ...
      pnlFromEntryCents >= this._breakEvenTriggerCents;
    const profitLockTargetHit =
      ...
      pnlFromEntryCents >= this._profitLockTriggerCents;
    ...
    const effectiveExitPolicyHit = data.exitPolicyExit && !holdToExpiryGuard;
