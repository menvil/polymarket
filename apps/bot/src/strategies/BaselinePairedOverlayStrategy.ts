/**
 * BaselinePairedOverlayStrategy — baseline directional entry + conditional paired overlay.
 *
 * @remarks
 * Идея:
 ...
 * @example
 * ```typescript
 * const strategy = new BaselinePairedOverlayStrategy({
 ...
 * Параметры стратегии.
 */
export interface BaselinePairedOverlayConfig {
  ...
  /** Composite + CEX bps пороги для сильного `A`-входа. */
  readonly strongEntryCompositeMin?: number;
  readonly strongEntryCexBpsMin?: number;
  /** Пороги для среднего `B`-входа; всё ниже считается `C`. */
  readonly mediumEntryCompositeMin?: number;
  readonly mediumEntryCexBpsMin?: number;
  /** В первые `strongEntryProtectMs` после `A`-входа overlay почти не вмешивается. */
  readonly strongEntryProtectMs?: number;
  /** Для `B/C`: насколько должен схлопнуться CEX сигнал относительно entry. */
  readonly tierBDecayFactor?: number;
  readonly tierCDecayFactor?: number;
  /** Порог bps для hard flip против позиции. */
  readonly hardFlipBps?: number;
  /** Разрешать paired-lock даже для tier A (обычно false). */
  readonly allowTierALock?: boolean;
  /** Минимальный guaranteed lock-profit для overlay. */
  readonly overlayPairedLockMinProfitCents?: number;
}

type OverlayEntryTier = 'A' | 'B' | 'C';
...
export class BaselinePairedOverlayStrategy extends BaseStrategy<CCData, CCAction> {
  public readonly id: StrategyId;
  public readonly name = 'BaselinePairedOverlayStrategy';
  ...
  private readonly _strongEntryCompositeMin: number;
  private readonly _strongEntryCexBpsMin: number;
  private readonly _mediumEntryCompositeMin: number;
  private readonly _mediumEntryCexBpsMin: number;
  private readonly _strongEntryProtectMs: number;
  private readonly _tierBDecayFactor: number;
  private readonly _tierCDecayFactor: number;
  private readonly _hardFlipBps: number;
  private readonly _allowTierALock: boolean;
  private readonly _overlayPairedLockMinProfitCents: number;
  ...
  private _entryTier: OverlayEntryTier = 'C';
  private _entryComposite = 0;
  private _entryCexAbsBps = 0;
  ...
  private _pendingOverlayBuyAtMs = 0;
  private _pendingOverlayBuyPriceCents = 0;
  private _pendingOverlayBuySide: EntrySide | undefined;
  private _overlayBuyUsed = false;

  constructor(
    config: BaselinePairedOverlayConfig,
    strategyId: StrategyId = unsafeStrategyId('baseline-paired-overlay-1'),
    logger?: ILogger,
    ...
    this._crowdGate = config.crowdGate ?? 'relaxed';
    ...
    this._relaxedEntrySide = config.relaxedEntrySide ?? 'up';
    ...
    this._cexFilterMode = config.cexFilterMode ?? 'not-adverse';
    this._cexSignalId = config.cexSignalId ?? 'cex_chainlink_linear_lead_lag';
    ...
    this._strongEntryCompositeMin = config.strongEntryCompositeMin ?? 0.35;
    this._strongEntryCexBpsMin = config.strongEntryCexBpsMin ?? 1.0;
    this._mediumEntryCompositeMin = config.mediumEntryCompositeMin ?? 0.15;
    this._mediumEntryCexBpsMin = config.mediumEntryCexBpsMin ?? 0.5;
    this._strongEntryProtectMs = config.strongEntryProtectMs ?? 20_000;
    this._tierBDecayFactor = config.tierBDecayFactor ?? 0.35;
    this._tierCDecayFactor = config.tierCDecayFactor ?? 0.6;
    this._hardFlipBps = config.hardFlipBps ?? 0.35;
    this._allowTierALock = config.allowTierALock ?? false;
    this._overlayPairedLockMinProfitCents = config.overlayPairedLockMinProfitCents ?? 0.25;

    this._logger?.warn('BaselinePairedOverlay: loaded', {
      ...
      overlay: {
        strongEntryCompositeMin: this._strongEntryCompositeMin,
        strongEntryCexBpsMin: this._strongEntryCexBpsMin,
        mediumEntryCompositeMin: this._mediumEntryCompositeMin,
        mediumEntryCexBpsMin: this._mediumEntryCexBpsMin,
        strongEntryProtectMs: this._strongEntryProtectMs,
        tierBDecayFactor: this._tierBDecayFactor,
        tierCDecayFactor: this._tierCDecayFactor,
        hardFlipBps: this._hardFlipBps,
        allowTierALock: this._allowTierALock,
        overlayPairedLockMinProfitCents: this._overlayPairedLockMinProfitCents,
      },
  ...
  protected gather(snapshot: StrategySnapshot): CCData | undefined {
    ...
    if (this._currentExpirationMs !== expiresMs) {
      ...
      this._entryTier = 'C';
      this._entryComposite = 0;
      this._entryCexAbsBps = 0;
      ...
      this._pendingOverlayBuyAtMs = 0;
      this._pendingOverlayBuyPriceCents = 0;
      this._pendingOverlayBuySide = undefined;
      this._overlayBuyUsed = false;
    ...
    if (anyPosition) {
      ...
      if (
        ...
        && !(complementary?.hasMatchedBuy ?? false)
        && !(complementary?.hasMatchedSell ?? false)
        && (this._pendingOverlayBuyAtMs <= 0 || (snapshot.nowMs - this._pendingOverlayBuyAtMs) >= PENDING_ORDER_GRACE_MS)
      ) {
        this._pendingOverlayBuyAtMs = 0;
        this._pendingOverlayBuyPriceCents = 0;
        this._pendingOverlayBuySide = undefined;
  ...
  protected decide(data: CCData): CCAction[] {
    ...
    // Overlay управляет только failure modes baseline entry.
    const exitTarget = this._resolveExitTarget(data);
    if (this._inPosition && exitTarget) {
      if (this._hasActiveOverlayLock(data)) {
        this._recordDecision(data, 'HOLD', 'paired-lock-active');
        return [{ type: 'STOP' }];
      }

      const pairedLock = this._buildOverlayPairedLock(data);
      if (pairedLock) return pairedLock;

      const exitReason = this._checkOverlayExitConditions(data);
      if (exitReason) {
        if (exitTarget.availableTokenQty.gt(0)) {
          ...
          if (exitTarget.openSellAtCents !== undefined) {
            this._recordDecision(data, 'HOLD', 'open-sell-working', {
              askCents: exitTarget.openSellAtCents,
              size,
              taker: takerExit,
            });
            return [];
          ...
          if (
            this._pendingOverlayBuySide !== undefined
            && (data.nowMs - this._pendingOverlayBuyAtMs) < PENDING_ORDER_GRACE_MS
          ) {
            this._recordDecision(data, 'HOLD', 'overlay-buy-pending', {
              bidCents: this._pendingOverlayBuyPriceCents || undefined,
              size,
              taker: takerExit,
            });
            return [];
          ...
          this._logger?.warn('BaselinePairedOverlay: EXIT', {
            side: exitTarget.side,
            reason: exitReason,
            tier: this._entryTier,
            ask,
            taker: takerExit,
    ...
    this._entryComposite = zone.weights.composite;
    this._entryCexAbsBps = Math.abs(this._alignedEntrySignalBps(data, entryTarget.side));
    this._entryTier = this._classifyEntryTier(zone, entryTarget.side, data);
    ...
    if (this._logEntries) {
      this._logger?.warn('BaselinePairedOverlay: ENTRY', {
  ...
  private _recordDecision(
    ...
    this._logger?.debug('BaselinePairedOverlay: decision', {
      action,
      rejectReason,
      bidPrice: order?.bidCents,
      askPrice: order?.askCents,
  ...
  private _checkOverlayExitConditions(data: CCData): string | undefined {
    const elapsedMs = this._entryPlacedAtMs > 0 ? data.nowMs - this._entryPlacedAtMs : 0;
    if (elapsedMs >= this._maxSignalHoldMs) return 'overlay-max-hold';

    if (!data.cexSignal || (this._cexRequireFresh && !data.cexFresh)) {
      if (!this._exitOnSignalStale) return undefined;
      return this._entryTier === 'A' && elapsedMs < this._strongEntryProtectMs
        ? undefined
        : 'signal-stale';
    }

    const direction = this._cexDirectionForEntrySide(data.cexSignal.direction, this._entrySide ?? 'UP');
    const alignedAbsBps = Math.abs(this._alignedEntrySignalBps(data, this._entrySide ?? 'UP'));
    const compositeNow = data.zone?.weights.composite ?? 0;
    const compositeRatio = this._entryComposite > 0 ? compositeNow / this._entryComposite : 0;

    if (this._exitOnTauTimeout && data.tauSec < this._exitTauSec) {
      return 'tau-timeout';
    }

    if (this._exitOnSignalFlip && direction === 'down' && alignedAbsBps >= this._hardFlipBps) {
      return 'signal-hard-flip';
    }

    if (this._entryTier === 'A' && elapsedMs < this._strongEntryProtectMs) {
      return undefined;
    }

    if (direction === 'flat' || alignedAbsBps <= this._normalizeThresholdBps) {
      return 'signal-normalized';
    }

    if (this._entryTier === 'B' && alignedAbsBps <= Math.max(this._normalizeThresholdBps, this._entryCexAbsBps * this._tierBDecayFactor)) {
      return 'signal-decay-b';
    }
    if (this._entryTier === 'C' && alignedAbsBps <= Math.max(this._normalizeThresholdBps, this._entryCexAbsBps * this._tierCDecayFactor)) {
      return 'signal-decay-c';
    }

    if (this._entryTier !== 'A' && this._entryComposite > 0 && compositeRatio <= 0.5) {
      return 'composite-decay';
    }

    if (this._entryTier === 'A' && this._entryComposite > 0 && compositeRatio <= 0.35) {
      return 'strong-entry-collapse';
    }
    ...
    if (this._exitOnRegimeFlip && this._entryRegime && data.regime) {
      ...
      if (opposite && this._entryTier !== 'A') return 'regime-flip';
    }
    if (this._exitOnWeightDrop && data.zone && data.zone.weights.composite < this._weightDropThreshold) {
      return 'weight-drop';
    }
    if (this._exitOnEdgeClosure && data.zone) {
      const edgeCentsNow = data.zone.train.edge * 100;
      if (Math.abs(edgeCentsNow) < this._edgeClosureCents && this._entryTier !== 'A') return 'edge-closure';
    }
    return undefined;
  }

  private _alignedEntrySignalBps(data: CCData, side: EntrySide): number {
    if (!data.cexSignal) return 0;
    const direction = this._cexDirectionForEntrySide(data.cexSignal.direction, side);
    if (direction === 'flat') return 0;
    return direction === 'up' ? Math.abs(data.cexSignal.value) : -Math.abs(data.cexSignal.value);
  }

  private _classifyEntryTier(zone: EdgeZone, side: EntrySide, data: CCData): OverlayEntryTier {
    const alignedBps = Math.abs(this._alignedEntrySignalBps(data, side));
    const composite = zone.weights.composite;
    if (composite >= this._strongEntryCompositeMin && alignedBps >= this._strongEntryCexBpsMin) return 'A';
    if (composite >= this._mediumEntryCompositeMin && alignedBps >= this._mediumEntryCexBpsMin) return 'B';
    return 'C';
  }

  private _buildOverlayPairedLock(data: CCData): CCAction[] | undefined {
    const exitReason = this._checkOverlayExitConditions(data);
    if (!exitReason) return undefined;
    if (this._entryTier === 'A' && !this._allowTierALock) return undefined;
    if (this._overlayBuyUsed) return undefined;
    if (
      this._pendingOverlayBuySide !== undefined
      && (data.nowMs - this._pendingOverlayBuyAtMs) < PENDING_ORDER_GRACE_MS
    ) {
    ...
    if (!hedgeTarget || hedgeTarget.bestAskCents === undefined) return undefined;
    if (hedgeTarget.positionQty.gt(0) || hedgeTarget.availableTokenQty.gt(0)) return undefined;
    if (hedgeTarget.hasOpenOrders || hedgeTarget.hasMatchedBuy || hedgeTarget.hasMatchedSell) return [];

    const targetShares = this._effectiveEntryShares(data);
    if (targetShares.lte(0)) return undefined;
    ...
    if (hedgeSize.lt(data.minOrderSize)) return undefined;

    const hedgeNotional = hedgeSize.mul(new Decimal(hedgePriceCents).div(100));
    if (hedgeNotional.lt(data.minOrderValue) || hedgeNotional.gt(data.availableBalance)) return undefined;

    const lockedProfitCents = this._lockedProfitCents(targetShares, hedgeSize, hedgePriceCents);
    const minLockProfit = Math.max(this._minLockedProfitCents, this._overlayPairedLockMinProfitCents);
    if (lockedProfitCents < minLockProfit) return undefined;

    if (hedgeTarget.openBuyAtCents === hedgePriceCents || hedgeTarget.hasMatchedBuy) return [];

    this._pendingOverlayBuyAtMs = data.nowMs;
    this._pendingOverlayBuyPriceCents = hedgePriceCents;
    this._pendingOverlayBuySide = hedgeTarget.side;
    this._overlayBuyUsed = true;

    this._logger?.warn('BaselinePairedOverlay: LOCK', {
      entrySide: this._entrySide,
    ...
    this._recordDecision(data, 'BUY', 'paired-lock', {
  ...
  private _buildHedgeExit(data: CCData, exitReason: string): CCAction[] | undefined {
    ...
    if (this._overlayBuyUsed) {
      this._recordDecision(data, 'HOLD', 'overlay-buy-already-used');
      return [{ type: 'STOP' }];
    }
    if (
      this._pendingOverlayBuySide !== undefined
    ...
    if (hedgeTarget.positionQty.gt(0) || hedgeTarget.availableTokenQty.gt(0)) {
      this._recordDecision(data, 'HOLD', 'hedge-already-open');
      return [{ type: 'STOP' }];
    }
    if (hedgeTarget.hasOpenOrders || hedgeTarget.hasMatchedBuy || hedgeTarget.hasMatchedSell) {
      this._recordDecision(data, 'HOLD', 'hedge-order-working');
      return [];
    }
    ...
    this._logger?.warn('BaselinePairedOverlay: HEDGE', {
  ...
  private _hasActiveOverlayLock(data: CCData): boolean {
    if (!this._overlayBuyUsed) return false;
    const hedgeTarget = this._resolveHedgeTarget(data);
    if (!hedgeTarget) return false;
    if (hedgeTarget.positionQty.gt(0) || hedgeTarget.availableTokenQty.gt(0)) return true;
    if (hedgeTarget.hasOpenOrders || hedgeTarget.hasMatchedBuy || hedgeTarget.hasMatchedSell) return true;
    return (
      this._pendingOverlayBuySide === hedgeTarget.side
      && (data.nowMs - this._pendingOverlayBuyAtMs) < PENDING_ORDER_GRACE_MS
    );
