 ...
 * фактически перемещённый USDC, и комиссия равна модулю его расхождения с
 * `size × price`. Сверено с документированной формулой
 * `size × 0.07 × p × (1 − p)` на всей истории — совпадение до пятого знака.
 *
 * Платит только тейкер, и платит из того, что получает:
 *
 * ```text
 * BUY  тейкер   отдаём номинал + fee    количество шар НЕ уменьшается
 * SELL тейкер   получаем номинал − fee
 * мейкер        ровно номинал           комиссии нет
 * ```
 *
 * `cashFlow` здесь строится из `amount`, то есть комиссия в нём уже учтена;
 * поле `fee` — объяснение величины, а не отдельное слагаемое.
 *
