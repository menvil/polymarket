 ...
 *
 * @deprecated
 * LEGACY REFERENCE ONLY — файл сохранён как историческая реализация и
 * ничем не собирается.
 *
 * - НЕ входит ни в один workspace: `legacy-bot/` не собирается, не
 *   типизируется и не линтуется;
 * - НЕ запускается против текущего репозитория: три импорта указывают на
 *   удалённый `@polymarket/exchange`;
 * - npm-скрипта `backfill:polymarket` больше НЕТ, он снят вместе с пакетом.
 *
 ...
 * Что переписать, чтобы он снова запускался, — в `README-backfill.md`.
 */
